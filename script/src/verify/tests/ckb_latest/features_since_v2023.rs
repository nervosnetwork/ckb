...
fn infinite_spawn_transaction(script_version: ScriptVersion) -> ResolvedTransaction {
    let (spawn_caller_cell, spawn_caller_data_hash) =
        load_cell_from_path("testdata/spawn_caller_exec");
    let (snapshot_cell, _) = load_cell_from_path("testdata/infinite_loop");
    let spawn_caller_script = Script::new_builder()
...
async fn check_txpool_active_budget_stops_and_joins_the_existing_vm_child() {
    ...
    let rtx = infinite_spawn_transaction(script_version);
    ...
    assert!(
        !probe
            .load_outside_slice
            .load(std::sync::atomic::Ordering::SeqCst),
        "root and dynamic program loading stay inside Running"
    );
    assert!(
...
async fn check_initial_suspend_defers_root_load_until_resume() {
    ...
    .await
    .expect("the child must observe the inherited Suspend before the assertions");
    assert!(
        !probe
            .slice_started
            .load(std::sync::atomic::Ordering::SeqCst),
        "an inherited Suspend must prevent root loading and VM execution"
    );
    assert!(
        !probe
            .root_load_started
...
async fn check_aborting_parent_terminates_the_suspended_vm_child() {
    assert_aborting_parent_terminates_vm_child(true).await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn check_closing_control_stops_suspended_budget_verification() {
    if SCRIPT_VERSION <= ScriptVersion::V1 {
        return;
    }
    let rtx = infinite_spawn_transaction(SCRIPT_VERSION);
    let probe = std::sync::Arc::new(VmChildTestProbe::default());
    let (command_tx, mut command_rx) = watch::channel(ChunkCommand::Suspend);
    let parent = tokio::spawn(VM_CHILD_TEST_PROBE.scope(
        std::sync::Arc::clone(&probe),
        async move {
            TransactionScriptsVerifierWithEnv::new()
                .verify_with_budget_async(
                    SCRIPT_VERSION,
                    &rtx,
                    &mut command_rx,
    ...
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        ...
        drop(command_tx);
        assert!(
            parent.await.unwrap().is_err(),
            "closed control stops verification"
        );
    })
    .await
    .expect("suspended verification must join when its controller disappears");
    assert!(!probe.active.load(std::sync::atomic::Ordering::SeqCst));
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn check_aborting_parent_terminates_the_running_vm_child() {
    assert_aborting_parent_terminates_vm_child(false).await;
}

#[test]
fn check_aborting_parent_terminates_the_single_worker_vm_child() {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(1)
        .enable_all()
        .build()
        .expect("the one-worker cancellation runtime must build");
    runtime.block_on(assert_aborting_parent_terminates_vm_child(false));
}

async fn assert_aborting_parent_terminates_vm_child(suspend: bool) {
    let script_version = SCRIPT_VERSION;
    if script_version <= ScriptVersion::V1 {
        return;
    }
    ...
    .await
    .expect("the VM must execute through its dynamic program load before cancellation");
    if suspend {
        command_tx
...
#[test]
fn check_txpool_budget_preserves_single_worker_parent_progress() {
    let script_version = SCRIPT_VERSION;
    if script_version <= ScriptVersion::V1 {
        return;
    }
...
#[test]
fn check_txpool_budget_preserves_completed_cycles() {
    let script_version = SCRIPT_VERSION;
