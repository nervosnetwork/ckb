...
pub mod ckb {
    ...
    pub mod protocol {
        ...
        impl<'a> Verify for reader::CellDep<'a> {
            fn verify(&self) -> Result {
                let tab = self._tab;
                let buf = tab.buf;
                let buf_len = buf.len();
                ...
                if Self::VT_IS_DEP_GROUP as usize + flatbuffers::SIZE_VOFFSET
                    <= vtab_num_bytes
                {
                    let voffset = vtab.get(Self::VT_IS_DEP_GROUP) as usize;
        ...
        impl<'a> Verify for reader::Transaction<'a> {
            fn verify(&self) -> Result {
                ...
                if Self::VT_CELL_DEPS as usize + flatbuffers::SIZE_VOFFSET
                    <= vtab_num_bytes
                {
                    let voffset = vtab.get(Self::VT_CELL_DEPS) as usize;
                    if voffset > 0 {
                        if voffset + 4 > object_inline_num_bytes {
                            return Err(Error::OutOfBounds);
                        }

                        let cell_deps_verifier = VectorVerifier::follow(
                            buf,
                            try_follow_uoffset(buf, tab.loc + voffset)?,
                        );
                        cell_deps_verifier
                            .verify_reference_elements::<reader::CellDep>()?;
                    }
                }

                if Self::VT_HEADER_DEPS as usize + flatbuffers::SIZE_VOFFSET
                    <= vtab_num_bytes
                {
                    let voffset = vtab.get(Self::VT_HEADER_DEPS) as usize;
                    if voffset > 0 {
                        if voffset + 4 > object_inline_num_bytes {
                            return Err(Error::OutOfBounds);
                        }

                        let header_deps_verifier = VectorVerifier::follow(
                            buf,
                            try_follow_uoffset(buf, tab.loc + voffset)?,
                        );
                        header_deps_verifier.verify_scalar_elements(32)?;
                    }
                }
