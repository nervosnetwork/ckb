...
impl<'a> InIBDProcess<'a> {
    ...
    pub fn execute(self) -> Status {
        ...
        if let Some(state) = self.synchronizer.peers().state.write().get_mut(&self.peer) {
            ...
            //
            // If outbound is an ibd node(non-whitelist, non-protect), it should be disconnected
            // automatically. If inbound is an ibd node, just mark the node does not
            // pass header sync authentication.
