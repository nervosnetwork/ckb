...
impl SyncShared {
    ...
    pub(crate) fn accept_remote_block(&self, chain: &ChainController, remote_block: RemoteBlock) {
        ...
        debug!("accept_remote_block {}-{}", block_number, block_hash);
        {
            let entry = self.shared().block_status_map().entry(block_hash.clone());
...
impl SyncState {
    ...
    pub fn shared_best_header_ref(&self) -> RwLockReadGuard<'_, HeaderIndex> {
