...
## [0.205.0-rc2](https://github.com/nervosnetwork/ckb/compare/v0.204.0...v0.205.0-rc2) - 2026-03-09
...
- Add consistent apt-get install step to ci_cargo_deny_ubuntu workflow (by @Copilot)
- Add cargo target caching and disk usage summary for non-protected branches (by @Copilot)
- Add disk cleanup steps to free up space on GitHub-hosted runners (by @Copilot)
- Add Rust toolchain setup step to ci_cargo_deny_ubuntu workflow (by @Copilot)
- Add workflow_dispatch trigger to all CI workflows for manual testing (by @Copilot)

### Changed

- *(release)* reset version before changelog generation in bump.sh
- release ([#5129](https://github.com/nervosnetwork/ckb/pull/5129)) (by @nervos-bot[bot])
- update `schemars` to version 1 ([#5128](https://github.com/nervosnetwork/ckb/pull/5128)) (by @Officeyutong)
- release ([#5064](https://github.com/nervosnetwork/ckb/pull/5064)) (by @nervos-bot[bot])
- disable semver check when creating release pr ([#5118](https://github.com/nervosnetwork/ckb/pull/5118)) (by @doitian)
...
- release ([#5052](https://github.com/nervosnetwork/ckb/pull/5052)) (by @nervos-bot[bot])
- Standardize apt-get install in ci_aarch64_build_ubuntu to match other workflows (by @Copilot)
- Revert "Remove conditional disk cleanup on Ubuntu workflows" (by @Copilot)
- Trigger workflows when pushing to any branch (by @Copilot)
- Make cancel-in-progress conditional for pull_request and workflow_dispatch only (by @Copilot)
- Use self-hosted runners for Linux workflows in nervosnetwork org (by @Copilot)
- Use GitHub-hosted runners for all CI workflows (by @Copilot)
- Update CI workflow documentation to reflect simplified structure (by @Copilot)
- Update all CI workflow files to remove prologue and inline logic (by @Copilot)
- bump to v0.205.0-pre (by @doitian)
...
- *(release)* read changelog from CHANGELOG.md for release issue ([#5122](https://github.com/nervosnetwork/ckb/pull/5122)) (by @doitian)
- update bytes from 1.10.1 to 1.11.1 (RUSTSEC-2026-0007) ([#5099](https://github.com/nervosnetwork/ckb/pull/5099)) (by @Copilot)
- fix macos runners in package workflow (by @doitian)
- Fix integration test log upload: set CKB_INTEGRATION_TEST_TMP and pack entire directory on failure (by @Copilot)

### Removed

- Remove conditional disk cleanup on Ubuntu workflows (by @Copilot)
- Remove CARGO_TARGET_DIR customizations from all workflows (by @Copilot)
