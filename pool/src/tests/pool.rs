...
use nervos_chain::chain::{ChainBuilder, ChainClient};
...
fn test_setup() -> (
    Arc<impl ChainClient>,
    Arc<TransactionPool<impl ChainClient>>,
    H256,
) {
...
fn apply_transactions(transactions: Vec<Transaction>, chain: &Arc<impl ChainClient>) -> Block {
    let time = now_ms();
    let header = Header {
        raw: RawHeader::new(
            &chain.tip_header(),
            transactions.iter(),
            time,
            cal_difficulty(&chain.tip_header(), time),
        ),
        seal: Seal {
            nonce: 0,
