...
impl ChainSpec {
    ...
    pub fn build_genesis(&self) -> Result<BlockView, Box<dyn Error>> {
        ...
        let block = BlockBuilder::default()
            .version(self.genesis.version.into())
            .parent_hash((&self.genesis.parent_hash).into())
            .timestamp(self.genesis.timestamp.into())
            .compact_target(self.genesis.compact_target.into())
            .extra_hash((&self.genesis.uncles_hash).into())
            .epoch(EpochNumberWithFraction::new_unchecked(0, 0, 0).into())
            .dao(dao)
            .nonce(u128::from_le_bytes(self.genesis.nonce.to_le_bytes()).into())
    ...
    fn build_cellbase_transaction(
        ...
        let special_issued_lock = packed::Script::new_builder()
            .args(secp_lock_arg(&Privkey::from(SPECIAL_CELL_PRIVKEY.clone())).into())
            .code_hash(CODE_HASH_SECP256K1_BLAKE160_SIGHASH_ALL.clone().into())
            .hash_type(ScriptHashType::Data.into())
            .build();
        let special_issued_cell = packed::CellOutput::new_builder()
            .capacity(special_cell_capacity.into())
    ...
    fn build_dep_group_transaction(
        ...
        let witness = Bytes::from(sig.serialize()).into();
...
impl SystemCell {
    fn build_output(
        ...
        let cell = if let Some(capacity) = self.capacity {
            let cell = builder.capacity(capacity.into()).build();
