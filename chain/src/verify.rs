...
/// The sole block consumer owns this pause across already queued work. Dropping
/// it on idle, shutdown or unwind restores pool computation; pool Stop remains
/// terminal. This does not wait for VM quiescence or pause chain publication.
struct VerificationPause<'a>(&'a TxPoolController);

impl<'a> VerificationPause<'a> {
    fn new(controller: &'a TxPoolController) -> Self {
        let _ = controller.suspend_chunk_process();
        Self(controller)
    }
}

impl Drop for VerificationPause<'_> {
    fn drop(&mut self) {
        let _ = self.0.continue_chunk_process();
    }
}
...
impl ConsumeUnverifiedBlocks {
    ...
    pub(crate) fn start(mut self) {
        let mut pool_pause = None;
        loop {
            // Keep the pause across a ready backlog, but release it before
            // waiting for more work. Keep using select! so truncate and stop
            // remain eligible between every pair of blocks.
            if self.unverified_block_rx.is_empty() && self.truncate_block_rx.is_empty() {
                pool_pause.take();
            }
            ...
            select! {
                recv(self.unverified_block_rx) -> msg => match msg {
                    Ok(unverified_task) => {
                        ...
                        pool_pause.get_or_insert_with(|| VerificationPause::new(&self.tx_pool_controller));
...
impl ConsumeUnverifiedBlockProcessor {
    ...
    fn monitor_block_txs_verified(
        ...
        if log_enabled_target!("ckb_tx_monitor", Trace) {
            // Completed results exclude cellbase; skip its resolved transaction
            // to align the monitor records.
...
#[cfg(test)]
#[path = "tests/verification_priority.rs"]
mod priority_tests;
