...
pub(crate) fn encode_v2(data: DiscoveryMessageV2) -> Bytes {
    data.encode()
}

pub(crate) fn decode_v2(data: &Bytes) -> Option<DiscoveryMessageV2> {
    DiscoveryMessageV2::decode(&data)
}
...
impl DiscoveryMessageV2 {
        ...
        let playload = match self {
            DiscoveryMessageV2::GetNodes {
                ...
                let listen_port = packed::PortOpt::new_builder()
                    ...
                    .build();
                let get_node = packed::GetNodesV2::new_builder()
                    .net(net.as_str().pack())
                    .listen_port(listen_port)
                    .count(count)
                    .version(version)
                    .build();
                packed::DiscoveryPayloadV2::new_builder()
                    .set(get_node)
                    .build()
            }
            DiscoveryMessageV2::Nodes(NodesV2 {
                net,
                announce,
                items,
            }) => {
                ...
                let items = packed::NodeVec::new_builder().set(item_vec).build();
                let nodes = packed::NodesV2::new_builder()
                    .net(net.as_str().pack())
                    .announce(announce)
                    .items(items)
                    .build();
                packed::DiscoveryPayloadV2::new_builder().set(nodes).build()
            }
        };

        packed::DiscoveryMessageV2::new_builder()
            .payload(playload)
            .build()
            .as_bytes()
    }
    ...
    pub fn decode(data: &[u8]) -> Option<Self> {
        let reader = packed::DiscoveryMessageV2Reader::from_compatible_slice(data).ok()?;
        match reader.payload().to_enum() {
            packed::DiscoveryPayloadV2UnionReader::GetNodesV2(reader) => {
                let version = {
                    let mut b = [0u8; 4];
                    b.copy_from_slice(reader.version().raw_data());
                    u32::from_le_bytes(b)
                ...
                Some(DiscoveryMessageV2::GetNodes {
                    net: reader.net().as_utf8().ok()?.to_owned(),
                    version,
                    count,
                    listen_port,
                })
            }
            packed::DiscoveryPayloadV2UnionReader::NodesV2(reader) => {
                let announce = match reader.announce().as_slice()[0] {
                    0 => false,
                    1 => true,
                    _ => return None,
                ...
                Some(DiscoveryMessageV2::Nodes(NodesV2 {
                    net: reader.net().as_utf8().ok()?.to_owned(),
                    announce,
                    items,
                }))
...
#[cfg(test)]
mod test {
    use super::{decode, decode_v2, encode, encode_v2, DiscoveryMessage, DiscoveryMessageV2};
    ...
    #[test]
    fn test_codec_v2() {
        let msg1 = DiscoveryMessageV2::GetNodes {
            net: "test".to_owned(),
            version: 0,
            count: 1,
            listen_port: Some(1),
        ...
        let msg2 = DiscoveryMessageV2::GetNodes {
            net: "test".to_owned(),
            version: 0,
            count: 1,
            listen_port: Some(2),
        ...
        let b1 = encode_v2(msg1.clone());

        let decode1 = decode_v2(&b1).unwrap();
        assert_eq!(decode1, msg1);

        let b2 = encode_v2(msg2.clone());

        let decode2 = decode_v2(&b2).unwrap();
        assert_eq!(decode2, msg2);
    }
}
