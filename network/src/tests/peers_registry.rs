...
fn test_accept_inbound_peer_eviction() {
    ...
    peers_registry
        ...
        .expect("accept");
    peers_registry
        .accept_inbound_peer(evict_target.clone(), addr1.clone(), session)
        .expect("accept");
    peers_registry
        .accept_inbound_peer(new_peer_ids[0].clone(), addr1.clone(), session)
        .expect("accept");
    peers_registry
        .accept_inbound_peer(new_peer_ids[1].clone(), addr1.clone(), session)
        .expect("accept");
    // setup 2 node from addr2
    peers_registry
        .accept_inbound_peer(lowest_score_peer.clone(), addr2.clone(), session)
        .expect("accept");
    peers_registry
        .accept_inbound_peer(new_peer_ids[2].clone(), addr2.clone(), session)
        .expect("accept");
    // setup score
    {
        let mut peer_store = peer_store.write();
        peer_store.report(&lowest_score_peer, behaviour::FAILED_TO_PING);
        peer_store.report(&lowest_score_peer, behaviour::FAILED_TO_PING);
        peer_store.report(&lowest_score_peer, behaviour::FAILED_TO_PING);
        peer_store.report(&reserved_peer, behaviour::FAILED_TO_PING);
        peer_store.report(&reserved_peer, behaviour::FAILED_TO_PING);
