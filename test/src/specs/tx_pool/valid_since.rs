...
impl ValidSince {
    ...
    // [current+cellbase_maturity, current+relative_number): Err(Immature)
    pub fn test_since_relative_block_number(&self, node: &Node) {
        ...
        for _ in self.cellbase_maturity()..relative {
            assert_send_transaction_fail(node, &transaction, "Immature");
            node.generate_block();
        }
    ...
    // (current, current+cellbase_maturity): Err(InvalidTx(CellbaseImmaturity))
    // [current+cellbase_maturity, absolute_number): Err(Immature)
