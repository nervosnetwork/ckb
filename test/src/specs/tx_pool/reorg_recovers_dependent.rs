...
/// again.
///
/// This exercises detached recovery alongside a child that remained accepted:
/// both must resolve against the new chain before returning to accepted membership.
...
impl Spec for ReorgRecoversDependentTxs {
    ...
    fn run(&self, nodes: &mut Vec<Node>) {
        ...
        commit_expected_transactions(node_a, std::slice::from_ref(parent));
        assert_eq!(node_a.get_transaction(child.hash()), TxStatus::pending());
        node_a.assert_tx_pool_size(1, 0);
        ...
        commit_expected_transactions(node_a, &[parent.clone(), child.clone()]);
...
///
/// This exercises dependency-ordered recovery from committed detached blocks.
...
/// After reorg recovers a committed dependent chain, its Pending projection
/// must feed normal `get_block_template` proposals and become committable again,
/// including when the miner omits an eligible uncle carrying the same proposals.
...
impl Spec for ReorgRecoversDependentPendingTree {
    ...
    fn run(&self, nodes: &mut Vec<Node>) {
        ...
        wait_for_pending_count(node_a, 3);
        // Check the pool's proposal position as well as the public RPC status.
        for tx in txs {
...
/// Commit the inspected template and establish the on-chain precondition;
/// `assert_new_block_committed` alone only checks template contents.
fn commit_expected_transactions(node: &Node, expected: &[TransactionView]) {
    assert_new_block_committed(node, expected);
    let block = node.new_block(None, None, None);
    assert_eq!(expected, &block.transactions()[1..]);
    node.submit_block(&block);
    for tx in expected {
        assert!(
            is_transaction_committed(node, tx),
            "transaction {:#x} must be committed before continuing",
            tx.hash()
        );
    }
}
