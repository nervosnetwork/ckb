...
impl Spec for SendLargeCyclesTxInBlock {
    ...
    fn modify_app_config(&self, config: &mut ckb_app_config::CKBAppConfig) {
        ...
        // A strict network budget must not reject local RPC submission or
        // canonical verification of the block relayed to the other node.
...
fn build_tx(node: &Node, privkey: &Privkey, lock_arg: Bytes) -> TransactionView {
    ...
    let witness = witness
        ...
        .build();
    tx.as_advanced_builder().witness(witness.as_bytes()).build()
}
