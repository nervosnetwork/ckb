...
pub enum DaoError {
    /// This error occurs during calculating the dao field for a block, which broadly indicates
    /// that it cannot find a required block.
    #[error("InvalidHeader")]
    InvalidHeader,

    /// When withdraws from NervosDAO, it requires the deposited header and withdrawing header to
    /// help calculating interest. This error occurs at [withdrawing phase 2] for the below
    /// cases:
    ///   - The [`HeaderDeps`] does not include the withdrawing block hash. The withdrawing block
    ///     hash indicates the block which packages the target transaction at [withdrawing phase
    ///     1].
    ///   - The [`HeaderDeps`] does not include the deposited block hash. The deposited block hash
    ///     indicates the block which packages the target transaction at [deposit phase]. Please
    ///     see [withdrawing phase 2] for more details.
    ///
    ...
    /// When withdraws from NervosDAO, the deposited header should be specified via witness. This
    /// error indicates the corresponding witness is unexpected. See
