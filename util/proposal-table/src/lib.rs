...
impl ProposalView {
    ...
    /// Constructs an exact view from materialized bands.
    pub fn new(
...
impl ProposalTable {
    ...
    /// Removes primitive ids at one height and invalidates incremental succession.
    ...
    fn rebuild_view(&self, number: BlockNumber) -> ProposalView {
        ...
        ProposalView::from_counts(counts)
    }
    ...
    fn successor_view(&self, origin: &ProposalView, number: BlockNumber) -> Option<ProposalView> {
        ...
        Some(ProposalView::from_counts(counts))
    }
