...
#[clap()]
pub struct CmdRun {
    #[clap(long, validator=is_hash256)]
...
/// init subcommand
#[derive(Args)]
#[clap(group(ArgGroup::new("ba").multiple(true).args(&["ba-arg", "ba-code-hash"])))]
pub struct CmdInit {
    #[clap(from_global)]
    ...
    #[clap(
        long,
        long_help = "Sets args in [block_assembler]",
        group = "ba",
    ...
    #[clap(long, long_help = "Sets message in [block_assembler]", requires("ba"))]
    ...
    #[clap(short, long)]
    /// Forces overwriting existing files
    pub force: bool,
    ...
    #[clap(short, long)]
    /// Lists available options for --chain
    list_chains: bool,
...
pub struct CmdReplay {
    ...
    #[clap(subcommand)]
    /// Replay Operation: profile or sanity_check
    replay_operation: ReplayOperation,
}

#[derive(Subcommand, Clone)]
/// replay subcommand
pub enum ReplayOperation {
    #[clap(about = "Profiling blockchain")]
    /// Enable profile
    Profile(CmdProfile),

    #[clap(about = "Sanity Check database")]
    /// Enable sanity check
    SanityCheck(CmdSanityCheck),
}

#[derive(Args, Clone)]
/// replay profile parameter
...
#[derive(Args, Clone)]
/// replay sanity check parameter
pub struct CmdSanityCheck {
    #[clap(long)]
...
impl Setup {
    ...
    pub fn from_matches(
        ...
        let root_dir = Self::root_dir_from_matches(&cfg)?;
    ...
    pub fn replay(self, matches: &CmdReplay) -> Result<ReplayArgs, ExitCode> {
        ...
        let mut profile_range = None;
        let mut sanity_check = false;
        let mut full_verification = false;

        match matches.replay_operation.clone() {
            ReplayOperation::Profile(profile) => {
                profile_range = if profile.from.is_some() || profile.to.is_some() {
                    Some((
                        Some(profile.from.unwrap_or_default()),
                        Some(profile.to.unwrap_or_default()),
                    ))
                } else {
                    None
                };
            }
            ReplayOperation::SanityCheck(sanitycheck) => {
                sanity_check = true;
                full_verification = sanitycheck.full_verification;
            }
        }
        // let sanity_check = matches.sanity_check.clone;
        // if let ReplayOperation::SanityCheck(sanitycheck) = matches.replay_operation.clone() {
        //     sanity_check= true;
        //     full_verification = sanitycheck.full_verification;
        // };
...
pub fn basic_app<'help>() -> Command<'help> {
    ...
    CKBSubCommand::augment_subcommands(cli)
}
...
fn is_hash256(hex: &str) -> Result<(), String> {
    if let Some(stripped) = hex.strip_prefix("0x") {
        let mut _target_h256 = [0u8; 32];
        match hex_decode(stripped.as_ref(), &mut _target_h256) {
            Ok(_) => Ok(()),
            _ => Err("Invalid H256 format, H256 string should be a hexadecimal string, starts with 0x and is 66 bytes long".to_owned()),
        }
    } else {
