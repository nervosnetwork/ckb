...
fn test_signing_alert_using_dummy_keypair() {
    ...
    let raw_alert = packed::RawAlert::new_builder()
        // 3 months later
        .notice_until(1681574400000u64.into())
        .id(20230001u32.into())
        .cancel(0u32.into())
        .priority(20u32.into())
        .message("CKB v0.105.* have bugs. Please upgrade to the latest version.".into())
        .min_version(Some("0.105.0-pre").into())
        .max_version(Some("0.105.1").into())
        .build();
    ...
    let alert = packed::Alert::new_builder()
        .raw(raw_alert)
        .signatures(vec![signature.clone()].into())
        .build();
