...
impl molecule::prelude::Entity for WitnessVec {
    ...
    fn from_compatible_slice(slice: &[u8]) -> molecule::error::VerificationResult<Self> {
        WitnessVecReader::from_compatible_slice(slice).map(|reader| reader.to_entity())
    }
    fn new_builder() -> Self::Builder {
        ::std::default::Default::default()
    }
...
impl WitnessVec {
    ...
    pub fn as_reader<'r>(&'r self) -> WitnessVecReader<'r> {
...
impl<'r> molecule::prelude::Reader<'r> for WitnessVecReader<'r> {
    ...
    fn verify(slice: &[u8], _compatible: bool) -> molecule::error::VerificationResult<()> {
        ...
        for i in 0..=(offsets.len() - 2) {
            let start = offsets[i];
            let end = offsets[i + 1];
            WitnessReader::verify(&slice[start..end], _compatible)?;
        }
        Ok(())
    }
}
impl<'r> WitnessVecReader<'r> {
    ...
    pub fn get(&self, idx: usize) -> Option<WitnessReader<'r>> {
        let len = self.len();
        if idx >= len {
            None
        } else {
    ...
    pub fn get_unchecked(&self, idx: usize) -> WitnessReader<'r> {
        let len = self.len();
        let (_, offsets) = self.offsets();
        let start = u32::from_le(offsets[idx]) as usize;
        if idx == len - 1 {
...
impl<'r> WitnessVecReader<'r> {
    pub fn iter<'t>(&'t self) -> WitnessVecReaderIterator<'t, 'r> {
...
impl<'r> WitnessReader<'r> {
    pub fn iter<'t>(&'t self) -> WitnessReaderIterator<'t, 'r> {
...
impl<'r> molecule::prelude::Reader<'r> for TransactionReader<'r> {
    ...
    fn verify(slice: &[u8], compatible: bool) -> molecule::error::VerificationResult<()> {
        ...
        WitnessVecReader::verify(&slice[offsets[1]..offsets[2]], compatible)?;
        Ok(())
    }
}
impl<'r> TransactionReader<'r> {
    ...
    pub fn witnesses(&self) -> WitnessVecReader<'r> {
        let (_, count, offsets) = Self::field_offsets(self);
        let start = u32::from_le(offsets[1]) as usize;
        if count == 2 {
