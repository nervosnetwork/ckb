...
impl Reject {
    ...
    /// `Duplicated` preserves existing status; unfinished verification is not retained.
    ...
    pub fn is_verification_interrupted(&self) -> bool {
        match self {
            Self::ExcessiveVerifyTime => true,
            Self::Verification(error) => {
                error.kind() == ErrorKind::Internal
                    && error
                        .downcast_ref::<InternalError>()
                        .is_some_and(|error| error.kind() == InternalErrorKind::Interrupts)
            }
            Self::LowFeeRate(..)
            | Self::Full(..)
            | Self::Duplicated(..)
...
mod tests {
    ...
    fn rejection_policies_preserve_recording_relay_and_peer_penalty_decisions() {
        let cases = [
            ...
            (
                Reject::Verification(InternalErrorKind::Interrupts.other("stopped").into()),
                (false, false, true),
            ),
            (
