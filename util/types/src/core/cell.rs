...
fn resolve_dep_group_with_cache<F: FnMut(&OutPoint, bool) -> Result<CellMeta, OutPointError>>(
    ...
    if let Some((dep_group_cell, resolved_deps)) = dep_cache.get(out_point) {
        ...
        let (dep_group_cell, resolved_deps) = resolve_dep_group(out_point, cell_resolver, eager_load)?;
        dep_cache.put(out_point.clone(), (dep_group_cell.clone(), resolved_deps.clone()));
