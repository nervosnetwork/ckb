...
impl TxVerificationCacheKey {
    ...
    /// and extension syscalls share [`header_visible_origin`] with this key.
    /// Changes to that contract must preserve the independent syscall visibility
    /// and cache identity tests, including invisible origins and source ordering.
    pub fn from_resolved(rtx: &ResolvedTransaction, script_rules: ScriptVerificationRules) -> Self {
        ...
        // Empty header deps skip visible-origin hashing, but the inline fingerprint
        // still participates in the cache key's size and ordinary Hash.
        if !headers.is_empty() {
            let headers: HashSet<_> = headers.into_iter().collect();
            let mut hasher = new_blake2b();
            // CellDep indexes the expanded members in resolved_cell_deps.
            // Dep-group containers have no separate syscall source index.
            for cells in [&rtx.resolved_inputs, &rtx.resolved_cell_deps] {
                hasher.update(&(cells.len() as u64).to_le_bytes());
                for cell in cells {
                    match header_visible_origin(cell, |hash| headers.contains(hash)) {
                        Some(hash) => {
                            hasher.update(&[1]); // A visible origin follows.
                            hasher.update(hash.as_slice());
                        }
                        None => hasher.update(&[0]), // ITEM_MISSING.
                    }
                }
            }
