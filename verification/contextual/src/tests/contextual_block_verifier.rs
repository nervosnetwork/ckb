...
fn disabled_script_verification_does_not_publish_cache_proof() {
    let (_chain, shared, parent_tx, always_success_script, always_success_out_point) = setup_env();
    ...
    let resolved = block
        ...
        .map(|transaction| {
            Arc::new(
                ...
                .expect("the block fixture resolves against genesis"),
            )
        })
        .collect::<Vec<_>>();
    ...
    let (handle, mut stopped, runtime) = ckb_async_runtime::new_global_runtime(Some(1));
    {
        let mmr = snapshot.chain_root_mmr(parent.number());
        let verifier = crate::ContextualBlockVerifier::new(
            dummy_context(&shared),
            &handle,
            Switch::DISABLE_ALL,
            Arc::clone(&cache),
            &mmr,
        );
        let (cycles, completed) = verifier
            .verify(&resolved, &block)
            .expect("assume-valid contextual transaction verification succeeds");
        assert_eq!(cycles, 0);
        assert_eq!(completed.len(), 1);
    }
    drop(handle);

    // Every task spawned by this verifier retains the runtime Handle's guard.
    // Channel closure therefore follows all possible cache writers, including
    // a writer that was spawned but had not yet been polled when verify returned.
    let cached = runtime.block_on(async {
        assert!(
            tokio::time::timeout(std::time::Duration::from_secs(5), stopped.recv())
                .await
                .expect("the verifier's spawned tasks must finish")
                .is_none()
        );
...
fn contextual_verification_does_not_reuse_cache_across_script_rules() {
    ...
    shared.async_handle().block_on(async {
        ...
        assert!(guard.lookup(&current_key).is_none());
        guard.insert(stale_proof);
    });
    ...
    // A reused stale proof cannot publish a proof for the new rule generation.
    // Observe the asynchronous writer's actual result; the timeout only bounds
    // failure and does not establish the execution order.
    let fresh = shared.async_handle().block_on(async {
        tokio::time::timeout(std::time::Duration::from_secs(5), async {
            loop {
                if let Some(proof) = cache.read().await.lookup(&current_key) {
                    break proof;
                }
        ...
        .await
        .expect("verification under current rules must publish fresh VM proof")
    });
    assert_eq!(fresh.key(), current_key);
}
