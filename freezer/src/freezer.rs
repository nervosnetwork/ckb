...
impl FreezerFiles {
    ...
    pub fn append(&mut self, number: u64, data: &[u8]) -> Result<(), Error> {
        ...
        if self.head.bytes + data_size as u64 > self.max_size {
            ...
            self.head = FreezerHead::new(new_head_file, 0);
        }
...
impl FreezerFilesBuilder {
    ...
    fn build(self) -> Result<FreezerFiles, Error> {
        ...
        let number = index_size / INDEX_ENTRY_SIZE - 1;

        Ok(FreezerFiles {
            files: BTreeMap::new(),
            head: FreezerHead::new(head, head_size),
            number,
