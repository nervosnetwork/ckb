...
continuously charged. The old coordinator/runtime/conflict-cache and accepted
rollback journal have been replaced by the seven-state `PrePoolKernel` and
accepted `PoolMutationPlan`; stable effects use one concrete statically
partitioned `EffectJournal` with no dynamic reservations. `TxPool` is the sole
accepted-state authority. Each phase must update a behavior row and its
evidence before deleting old encoding; derived indexes never become owners.
...
- Identify the linearization point and prove there is no visible ownership gap
  or overlap.
- Write the lock/resource order explicitly. The current mutation order is
  `optional driver/persistence permit -> TxPool -> PrePoolKernel -> EffectJournal`;
  capacity waits own no state and occur before authority locks, and no
  authority guard spans work/I/O/await. Detached replay is charged,
  persistable `RecoveryRetained` data; there is no recovery-wide lock or
  handler-owned transaction vector after the paired first phase.
...
`make integration CKB_TEST_ARGS='-c 1 CellBeingCellDepAndSpentInSameBlockTestGetBlockTemplate TxPoolLimitAncestorCount ReorgRecoversDependentPendingTree ReorgRecoversDependentChain ReorgRecoversDependentTxs PoolResolveConflictAfterReorg RbfRejectReplaceProposed RbfOrphanRecovery RbfBasic RbfReplaceProposedSuccess RbfConcurrency RbfCyclingAttack RbfCellDepsCheck'`
...
| `TP-DEP-001` Causal dependency graph | `tx-pool/src/component/pre_pool/wait.rs`<br>`tx-pool/src/component/pre_pool/lifecycle.rs`<br>`tx-pool/src/resolved_tx.rs`<br>`tx-pool/src/component/links.rs`<br>`tx-pool/src/pool_cell.rs`<br>`tx-pool/src/component/tx_selector.rs` | Raw, resolved and accepted exact dependency keys—including headers and expanded dep-group members—remain one canonical primary fact: availability wakes children and definitive loss invalidates them atomically while by-parent is only a derived projection. | Late-discovered parents, conditional reader/spender cycles, reversed arrival order, stale resolved children or parent replacement must not strand a child, lose its wake edge or let a template commit an invalid order. | I1, I4, I5, I6, I7, I9, I10, I11, I12 | - Are input, cell-dep, header-dep and expanded dep-group roles intentionally distinguished?<br>- Does parent success/failure update reverse edges, accounting and child location in one transition?<br>- Are cascade size and maintenance work explicitly bounded? | Use bounded indexed parent/child buckets and maintenance slices; never poll all waiting transactions or scan the pool for dependents. |
...
| `TP-WORKER-001` Level-triggered executable readiness | `tx-pool/src/component/pre_pool/queue.rs`<br>`tx-pool/src/component/pre_pool/runtime.rs`<br>`tx-pool/src/service/workers.rs`<br>`tx-pool/src/service/builder.rs`<br>`tx-pool/src/worker.rs` | Readiness is derived after each transition from the authoritative capability-aware checkout predicate; failed ineligible checkout is silent and one supervisor per worker re-arms executable work after a panic with shared bounded backoff. Cancellation and loss of command authority are terminal. | A capped peer backlog must not self-wake into mutex-starving livelock; a small-only worker must not consume the only wake for large work; cancellation, zero workers, a consumed Notify before panic or a permanently closed command channel must neither strand executable work nor create an immediate respawn loop. | I4, I12 | - Does notification mean at least one worker of this capability can execute now?<br>- Can a consumed permit be reconstructed from authoritative state after subscribe or respawn?<br>- Does cancellation stop checkout before another self-sustaining wake loop begins? | Readiness checks inspect bounded owner heads/caps only; worker count is configuration-bounded and each worker has exactly one supervisor with bounded restart rate; no manager task, exit channel, polling loop, per-item task or queue-wide scan. |
| `TP-ADMIN-001` Administrative and hostile-peer terminalization | `tx-pool/src/service.rs`<br>`tx-pool/src/service/pipeline_ops.rs`<br>`tx-pool/src/component/recent_reject.rs` | Ban, expiry and malformed-input policy terminalize the current non-committing owner once, release all resource/index state and publish only policy-eligible effects. Clear swaps the complete entry generation and publishes one constant-size GenerationReset, which releases all relayer filters without scanning or retaining a population-sized hash batch. | An already active malicious peer must not pin an active slot, resurrect through a late lease or turn a typed malformed input into fail-stop or an ineligible relay reject. | I4, I5, I8, I12 | - Does administrative removal make every outstanding lease stale atomically?<br>- Are trusted promotion and immutable ingress attribution preserved deliberately?<br>- Are ban/reject history and relay policy separate explicit decisions? | Administrative removal uses indexed owners and bounded batches; peer fences/history remain bounded and expiring. |
| `TP-EFFECT-001` Statically partitioned stable-state effects | `tx-pool/src/service/effects.rs`<br>`tx-pool/src/service/builder.rs`<br>`tx-pool/src/callback.rs` | A mutation-coupled immutable batch passes one exact static-region predicate, runs a total Apply and enters the global sequence under the journal's innermost lock; no reservation or capacity token crosses a lock or await. Remote, trusted and chain-critical regions are isolated, resident queued/active records are exactly charged, and a chain transition that cannot publish detail installs one prebuilt replaceable GenerationReset without waiting behind the FIFO. | Remote saturation, a full critical FIFO, a permanently full relay endpoint, callback re-entry/panic/hang, publisher unwind, close race or check-before-wait window must not mutate without a matching authority record, consume trusted/critical progress, delay chain convergence, expose intermediate state, reorder a retained batch, lose charge, spawn unbounded workers, fail-stop the service or sleep forever. | I5, I7, I8, I12 | - Is the immutable batch exact before Apply, or covered by one statically proved largest-indivisible Plan bound?<br>- Does Full/Closed/Oversize return before Apply, with the journal lock innermost and no capacity token crossing a lock or await?<br>- Are Remote, trusted and critical usage accounted independently while queued and active, and does saturated chain detail fall back to the prebuilt reset register rather than wait? | One bounded journal publisher and one bounded callback endpoint worker; O(1) admission/accounting, no per-effect task, unbounded channel, callback-under-lock or busy retry. |
| `TP-REORG-001` Serialized reliable chain transitions | `tx-pool/src/process/reorg.rs`<br>`tx-pool/src/component/pre_pool/recovery.rs`<br>`tx-pool/src/service.rs`<br>`tx-pool/src/service/pipeline_ops.rs` | The first retained reorg phase atomically switches snapshot/membership, transfers the complete bounded accepted descendant closure of detached producers into one charged parent-first RecoveryRetained session, then journals the immediate template reset; its trusted drainer uses ordinary admission while the independently retried second phase carries only a bounded compact-uncle token and publishes the matching final generation. | Repeated phase panic, a late accepted child, startup zombie reconcile, an over-budget or over-fanout descendant closure, duplicate/colliding detached roots, clear/save races or a stale full-template retry must not drop/overtake chain authority, expose a parentless accepted suffix or unowned payload interval, retain transaction-bearing blocks outside the kernel, cross a missing parent prefix or publish stale uncles/template state. | I1, I2, I4, I5, I6, I7, I8, I10, I11, I12 | - Does the paired mutation follow TxPool then PrePoolKernel then innermost EffectJournal, with every recovery payload charged before locks open and no stateful guard crossing an await?<br>- Can phase-two retry replay phase one, retain full detached blocks, or leave its inserted uncles after panic/supersession?<br>- Are accepted descendants planned before startup zombie reconciliation, transferred child-first/retained parent-first, and reset as one ephemeral generation when the bounded closure cannot fit?<br>- Are duplicate/collision handling, closure-safe truncation, full/witness identity, non-reused session/version clocks and clear epoch ordering exact? | First-phase graph traversal and transfer stop at the fixed mutation/recovery bounds and use one generation reset beyond them; phase two contains at most the assembler candidate bound, with no recovery lock, independent worker, population scan or full-block duplication after phase one. |
...
| `TP-DEFECT-001` Generation-local defect containment | `tx-pool/src/component/pre_pool/runtime.rs`<br>`tx-pool/src/process/mod.rs`<br>`tx-pool/src/process/submit/mod.rs`<br>`sync/src/relayer/mod.rs` | An impossible authoritative unwind exchanges only ephemeral accepted/entry generations while the TxPool guard remains closed, preserves process-global version/session clocks, emits one prebuilt GenerationReset and keeps chain, Local, Proposal, query and mining paths live; the sealed retired accepted/kernel bundle is destroyed only after authority guards open and replenishes the single emergency spare, while Remote alone enters bounded cooling, exact-witness quarantine and circuit-open after repeated resets. | A reproducible legal witness, stale pre-reset lease, session reuse, relay saturation or repeated witness variants must not cancel the tx-pool service, persist/reuse a partial generation, ABA-delete a new owner or force an endless whole-pool reset loop. | I1, I4, I5, I7, I8, I12 | - Does the swap install the target/current chain snapshot and fresh primary/index generation before either authority guard opens?<br>- Do entry versions, recovery sessions and stable effect/template authorities survive the generation exchange?<br>- Does the one disposal permit keep retired residency bounded and force destruction/spare replenishment outside both authority locks?<br>- Is exact wtx quarantine/cooling absent from the healthy hot-path lock set, bounded in memory, and fail-closed only for Remote after repeated resets? | Healthy admission adds no new mutex acquisition; one prebuilt spare, one prebuilt reset record and one serialized disposal permit make the impossible swap bounded without destroying population-sized graphs under authority locks. |
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(pipeline_commit_worker|local_commit_waits|target_model_exercises_every_plan_outcome|sparse_plan_matches)/)'`
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(parent_commit_before_wait|parent_loss|successive_expanded_parent_losses|dependency_epochs|dep_reader|selected_reader|conditional_cycle|dep_group|unknown_outpoint)/)'`
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(admission_budget_failure|parent_loss_uses_the_continuous_wait_reservation|remote_conflict_keeps_remote_reservation|owner_fairness_and_active_caps|count_and_byte_limits|conflict_closure_aborts|self_eviction_plan)/)'`
...
- `supervisor_does_not_respawn_after_command_channel_dropped` (I4, I12)
- `worker_exits_when_command_channel_dropped` (I4, I12)
- `zero_verify_worker_config_still_runs_remote_pipeline` (I4)

#### `TP-ADMIN-001` — Administrative and hostile-peer terminalization

Minimum command: `cargo nextest run -p ckb-tx-pool --features internal -E 'test(/(banned_peer_revokes|malformed_remote_preflight|proposal_promoted_remote_clear)/)'`
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(journal_usage|journal_sequence|full_journal|remote_byte_ceiling|critical_headroom|authoritative_apply|generation_reset_register|permanently_full_relayer|hung_callback|replacement_publisher|close_wakes|idle_publisher)/)'`
...
- `permanently_full_relayer_has_bounded_fifo_residency` (I5, I8, I12)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal -E 'test(/(reorg_status_transition|accepted_reorg_recovery|reorg_replays_detached_parent|over_bound_reorg|recovery_batch|empty_generation_recovery|second_phase_retry|retained_receiver|clear_during_reorg|cross_authority_query)/)'`
...
- `recovery_batch_is_atomic_parent_first_and_persistable_while_leased` (I1, I5, I7)
...
- `second_phase_retry_never_replays_completed_first_phase` (I7, I11)
...
#### `TP-DEFECT-001` — Generation-local defect containment

Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(authoritative_defect|authoritative_generation_swap|chain_generation_reset)/)'`

Rust evidence:

- `authoritative_defect_is_generation_local_not_service_fail_stop` (I4, I5, I8, I12)
- `authoritative_generation_swap_preserves_aba_clocks` (I1, I4, I7)
- `chain_generation_reset_reuses_lock_outside_disposal_without_remote_cooling` (I4, I5, I7, I8)
- `generation_reset_register_bypasses_full_fifo_and_coalesces` (I4, I5, I8)
- `structural_recovery_defect_advances_reorg_without_service_fail_stop` (I4, I7, I8)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(reorg_demotes_stale_gap|pending_proposals_filter|full_and_uncle_updates|selected_reader|conditional_cycle)/)'`
