...
- Write the lock/resource order explicitly. The current mutation order is
  `optional recovery/driver guard -> TxPool -> PrePoolKernel -> EffectJournal`;
  capacity waits own no state and occur before authority locks, and no
  authority guard spans work/I/O/await. `recovery_lock` remains a compatibility
  barrier only until P4 installs charged `RecoveryRetained` ownership; no new
...
`make integration CKB_TEST_ARGS='-c 1 CellBeingCellDepAndSpentInSameBlockTestGetBlockTemplate TxPoolLimitAncestorCount ReorgRecoversDependentPendingTree ReorgRecoversDependentChain ReorgRecoversDependentTxs RbfRejectReplaceProposed RbfOrphanRecovery RbfBasic RbfReplaceProposedSuccess RbfConcurrency RbfCyclingAttack RbfCellDepsCheck'`
...
| `TP-EFFECT-001` Statically partitioned stable-state effects | `tx-pool/src/service/effects.rs`<br>`tx-pool/src/service/builder.rs`<br>`tx-pool/src/callback.rs` | A mutation-coupled immutable batch passes one exact static-region predicate, runs a total Apply and enters the global sequence under the journal's innermost lock; no reservation or capacity token crosses a lock or await. Remote, trusted and chain-critical regions are isolated, resident queued/active records are exactly charged, and bounded publication occurs outside authority locks. | Remote saturation, a permanently full relay endpoint, callback re-entry/panic/hang, publisher unwind, close race or check-before-wait window must not mutate without a matching record, consume trusted/critical progress, expose intermediate state, reorder a retained batch, lose charge, spawn unbounded workers, fail-stop the service or sleep forever. | I5, I8, I12 | - Is the immutable batch exact before Apply, or covered by one statically proved largest-indivisible Plan bound?<br>- Does Full/Closed/Oversize return before Apply, with the journal lock innermost and no capacity token crossing a lock or await?<br>- Are Remote, trusted and critical usage accounted independently while queued and active, and are endpoint failure/circuit dispositions bounded? | One bounded journal publisher and one bounded callback endpoint worker; O(1) admission/accounting, no per-effect task, unbounded channel, callback-under-lock or busy retry. |
| `TP-REORG-001` Serialized reliable chain transitions | `tx-pool/src/process/reorg.rs`<br>`tx-pool/src/service.rs`<br>`tx-pool/src/service/pipeline_ops.rs` | Reorg deltas remain ordered and retained through retry; recovery_lock serializes complete detached replay, clear and persistence, while final effects describe only authoritative post-replay state. | Repeated callback panic, duplicate detached roots, clear/save races or partial replay must not drop/overtake a delta, expose an ownership gap, block on a capacity resource held across recovery or persist an intermediate pool. | I1, I2, I4, I6, I7, I8, I11 | - Is the compatibility order recovery_lock then TxPool then PrePoolKernel then innermost EffectJournal preserved, with capacity waits owning no state?<br>- Can retry replay an already completed authoritative phase or an obsolete snapshot?<br>- Are duplicate recovery, final-state effects and attached/detached identities exact? | Retain a capacity-one ordered delta and bounded replay slices; no independent recovery worker or full-history duplication. |
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(journal_usage|journal_sequence|full_journal|remote_byte_ceiling|critical_headroom|permanently_full_relayer|hung_callback|replacement_publisher|close_wakes|idle_publisher)/)'`
