...
- Prove every legal outcome is `Apply`, typed `Reject`, `Backpressure`, `Stale`
  or `Duplicate`. Untrusted job/endpoint panics are contained only at their
  boundary; primary/index/accounting contradictions fail fast and skip
  persistence rather than entering a repair/restart protocol.
...
| `TP-WORKER-001` Level-triggered executable readiness | `tx-pool/src/component/pre_pool/queue.rs`<br>`tx-pool/src/component/pre_pool/runtime.rs`<br>`tx-pool/src/service/workers.rs`<br>`tx-pool/src/service/builder.rs`<br>`tx-pool/src/worker.rs` | Readiness is derived after each transition from the authoritative capability-aware checkout predicate; failed ineligible checkout is silent, a job-computation panic settles that borrowed job and re-arms retained work, and loss of worker/command authority requests controlled service shutdown rather than model restart. | A capped peer backlog must not self-wake into mutex-starving livelock; a small-only worker must not consume the only wake for large work; cancellation, zero workers, a consumed Notify around a job panic or a permanently closed command channel must neither strand executable work nor create a respawn loop. | I4, I12 | - Does notification mean at least one worker of this capability can execute now?<br>- Can a consumed wake be reconstructed from authoritative state after subscribe or a contained job panic?<br>- Does cancellation stop checkout before another self-sustaining wake loop begins? | Readiness checks inspect bounded owner heads/caps only; worker count is configuration-bounded; no restart generation, polling loop, per-item task or queue-wide scan. |
...
| `TP-DEFECT-001` Rust-native failure and persistence boundary | `tx-pool/src/component/pre_pool/runtime.rs`<br>`tx-pool/src/process/mod.rs`<br>`tx-pool/src/process/submit/mod.rs`<br>`sync/src/relayer/mod.rs` | All legal malformed, policy, capacity, duplicate and stale outcomes remain typed before Apply; untrusted job/endpoint panics are contained at their boundary, while a primary/index/accounting contradiction fails fast, cancels the service and makes graceful persistence ineligible rather than repairing or persisting unknown state. | A reproducible legal transaction, stale lease, endpoint panic/hang or relay saturation must not reach the structural assertion boundary; a genuine internal contradiction must not be converted to success/rejection, restarted into an unknown generation or written to persistence. | I1, I4, I5, I7, I8, I12 | - Can every peer-selectable legal outcome be traced to a typed result before an assertion?<br>- Are catch_unwind boundaries limited to borrowed computation and external endpoints rather than authority repair?<br>- Does an invariant failure cancel supervised state work and skip persistence?<br>- Are ordinary clear/over-bound chain generation swaps kept distinct from invariant-panic recovery? | Healthy admission adds no recovery mutex, quarantine lookup, spare generation or restart protocol; job/endpoint unwind guards stay at existing bounded boundaries. |
...
- `catch_job_panic_captures_panic_and_lets_work_continue` (I4, I12)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(journal_usage|journal_sequence|full_journal|remote_byte_ceiling|critical_headroom|authoritative_apply|generation_reset_register|full_relayer|hung_callback|replacement_publisher|close_wakes|idle_publisher)/)'`
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(journal_accounting_drift|authoritative_generation_swap|chain_generation_reset|panicked_effect_publisher)/)'`
...
- `journal_accounting_drift_fails_fast_instead_of_repairing` (I4, I5, I8, I12)
...
- `status_counter_underflow_fails_fast_instead_of_masking_corruption` (I10)
- `test_dep_group` (I10)
