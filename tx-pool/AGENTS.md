...
## Active txpool-v8 long-running handoff

When a task explicitly resumes the txpool-v8 true-shard architecture goal,
read `docs/handoff/txpool-v8/README.md` and its manifest-bound first-read set
before using chat history, older checkpoints or temporary worktrees. Run its
verifier before changing code. That repository-owned handoff is the sole live
control-plane pointer; content-addressed external materials are evidence only.
