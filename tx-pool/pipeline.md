...
| S4 | Complete (checkpoint is the commit containing this row) | Compacted only test encoding: repeated remote-ingress construction and accepted-pool waits now use small test-only helpers with cycles, peer, expected count and timeout still explicit; repeated proposal fixtures retain fee, size and status at each call site; coordinator apply-fault cases share cleanup without merging their named failure boundaries; only the genuinely repeated deep-chain shape is shared in the RBF regressions. All 337 test names, 75 executable Rust anchors / 112 invariant references and 10 integration anchors remain exact. Independent test source fell from 21,010 to 20,541 lines (−469, 2.23%) without a mega-test, production audit reuse, visibility widening or any production file change. Whole-architecture review therefore found no change to ownership, causal exits, locks/waits, budgets, effect ordering, RPC, reorg/template/persistence or attacker-controlled work; `TP-WORKER-001` and `TP-EFFECT-001` retain the permanent livelock/lost-wakeup family. Fresh gates: 337/337 internal nextest in 25.906s, zero-warning all-target clippy, format/diff and all three guide/layout/security validators. The phase review deleted a low-value generic RBF fixture whose line saving did not justify its review diff; further test DSL was rejected because it would hide security parameters for a line-count target. Benchmarking remains S7 only. |
