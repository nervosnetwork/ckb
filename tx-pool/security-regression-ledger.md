...
| 104 | Model-covered | A bounded entry undo guard wraps every current multi-entry apply family. Boundary injection covers dependency schedule/drain, parent wake/demotion, accepted-input park/wake, verified preemption, conflict recheck single/batch, owner removal, expiry batch, plain/candidate handoff and their causal child outcomes; clear reserves output before ownership mutation. Every unwind restores authoritative entries, budgets and derived indexes, preserves surviving peer rotation, and exposes no partial result before retry. Repeat the same fault matrix across the TxPool/coordinator/outbox production transaction at cutover. | I1, I4 |
...
| 111 | Open | Dependency-failure and conflict-recheck maintenance order is not yet encoded in authoritative entry state, so rebuilding derived queues can change observable processing order. Add monotonic maintenance sequence keys and audit/rebuild exact order before production cutover. | I1, I4, I6 |
| 112 | Open | Current peer round-robin is a fairness policy change from configured FIFO/fee ordering, and worker capability such as `OnlySmallCycle` is not modeled independently from queue order. Separate eligibility/capability from policy and preserve externally configured scheduling semantics. | I1, I5, I12 |
| 113 | Open | Per-parent/per-input first-fill caps can be monopolized by remote low-value entries, excluding proposal, local, or strictly better verified work without exceeding any global budget. Define protected reconciliation capacity and deterministic bounded eviction/preemption rather than an untrusted first-come reservation. | I5, I6, I9, I12 |
