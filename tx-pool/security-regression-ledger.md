...
| O8 | Evidence debt | The current production-evidence table and new rows are checked manually. The historical table intentionally preserves names from deleted WaitingRoom/queue code and therefore cannot be treated as a compile-time anchor list. Add a machine-readable current-test manifest/CI existence check before using this ledger alone as release attestation; until then full test commands and integration logs remain the authority. |
