...
| O2 | The permit now owns its issuing queue and outbox commit atomically shrinks, sequences and enqueues, deleting every bind/order intermediate failure. The remaining post-mutation contract is that conservative submit/reorg batch estimates must cover the actual immutable batch. | Prove the two variable-size estimators against their pool/RBF bounds (including adversarial reject payloads), or move their batch preparation inside an explicit rollback-capable state/effect transaction. |
...
5. Workload, sample count, safety checks, and timeouts cannot be weakened to
   pass the gate.
6. O1, O2, O4, and O5 must be explicitly resolved or accepted with a documented fail-stop
