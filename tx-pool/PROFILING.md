...
- scheduler cursor-free/fairness hidden-stage wait and hold lifetimes, including
  writer-preferred gate contention.

All Local Direct final dispositions are attributed to the centralized
authority read wait/hold spans. A capture that exercises this path must not
attribute its terminal to an authority write hold: accepted owner/RBF/resource
cuts, exact duplicate/rejection read cuts and sole-journal contention remain
inside that read-held interval. Existing one-shot scenarios do not isolate
these Direct shapes, so they cannot support a standalone performance claim for
the migration.
...
Profiles guide candidate engineering. Final performance claims require a
result-before-frozen candidate generation and freshly frozen `develop`
baseline, precommitted scenarios, noise rule and practical non-inferiority
margins. A prior true-shard identity may be measured only as an attribution
diagnostic; it is not an additional delivery candidate unless the generation
contract explicitly admits it before results are observed.
