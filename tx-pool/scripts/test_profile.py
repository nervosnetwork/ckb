...
class ProfileAnalyzerTests(unittest.TestCase):
    ...
    def test_one_shot_reverse_observation_binds_unknown_parent_multiset(self) -> None:
        expected = {
            "scenario": "dependent_forest_8_reverse",
            "target": 4,
            "warm": 0,
            "workers": 2,
            "peers": 2,
        ...
        stdout = f"{PROFILE.OBSERVATION_PREFIX}{json.dumps(observation)}\n"
        self.assertEqual(PROFILE.parse_observation(stdout, expected), observation)
        with self.assertRaisesRegex(PROFILE.ProfileError, "count does not match"):
            PROFILE.parse_observation(
                f"{PROFILE.OBSERVATION_PREFIX}{json.dumps({**observation, 'relay_unknown_parents': 1})}\n",
                expected,
            )
        with self.assertRaisesRegex(PROFILE.ProfileError, "unknown-parent terminals"):
            nonreverse = {**observation, "scenario": "dependent_forest_8"}
            PROFILE.parse_observation(
                f"{PROFILE.OBSERVATION_PREFIX}{json.dumps(nonreverse)}\n",
                {**expected, "scenario": "dependent_forest_8"},
            )
