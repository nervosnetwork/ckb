...
BUILD = re.compile(
    ...
    r"callback_observer=(?P<callback_observer>\S+) "
...
def consensus_dependency_identity(root: Path, build_features: str) -> dict[str, object]:
    """Bind the exact CKB-VM source packages and enabled feature sets."""
    ...
    for name in CONSENSUS_LOCK_PACKAGES:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"expected exactly one {name} package in {root}/Cargo.lock"
            )
        package = matches[0]
        locked[name] = {
            key: package.get(key)
    ...
    if build_features:
        metadata_command.extend(("--features", build_features))
    try:
        metadata = json.loads(
    ...
    for node in nodes:
        ...
        if package is not None:
            enabled_features[package["name"]] = sorted(node.get("features", []))
    if set(enabled_features) != set(CONSENSUS_LOCK_PACKAGES):
...
def corpus_observation_error(
    corpus: object, expected_transactions: int
) -> str | None:
    if not isinstance(corpus, dict):
        return "benchmark corpus identity is unavailable"
    if set(corpus) != CORPUS_KEYS:
        return "benchmark corpus identity has an unsupported shape"
    if (
        corpus["transaction_count"] != expected_transactions
        or corpus["cycle_assignment_count"] != expected_transactions
    ):
        return "benchmark corpus transaction/cycle assignment count differs"
    script_preflight_count = corpus["script_preflight_count"]
    if (
        not isinstance(script_preflight_count, int)
        or isinstance(script_preflight_count, bool)
        or not 1 <= script_preflight_count <= expected_transactions
    ):
        return "benchmark script preflight count is invalid"
    cycles_sum = corpus["cycles_sum"]
    if (
        not isinstance(cycles_sum, int)
        or isinstance(cycles_sum, bool)
        or cycles_sum <= 0
    ):
    ...
    for field in (
        ...
        if not isinstance(corpus[field], str) or HEX_32.fullmatch(corpus[field]) is None:
            return f"benchmark corpus {field} is not a 32-byte lowercase hex digest"
    return None


def paired_corpus_error(
    baseline_corpus: object, candidate_corpus: object
) -> str | None:
    if baseline_corpus != candidate_corpus:
        return "baseline and candidate benchmark corpus identities differ"
    return None
...
def terminal_record_error(
    ...
    *,
    ...
    if not isinstance(terminals, dict):
        return "benchmark terminal identity is unavailable"
    if set(terminals) != TERMINAL_KEYS:
    ...
    if any(terminals[field] != expected for field, expected in scalar_fields.items()):
    ...
    normalized: list[tuple[int, tuple[str, ...], int]] = []
    ...
    if normalized != sorted(normalized) or len(normalized) != len(
        {(peer, parents) for peer, parents, _ in normalized}
    ):
        return "benchmark unknown-parent multiset is not canonical"
    if sum(count for _, _, count in normalized) != relay_unknown_parents:
...
def ac_power_mode_disabled(power_settings: str) -> bool:
    """Accept the legacy and current ``pmset`` names for AC power mode."""
    ac_section = power_settings.partition("AC Power:")[2]
    if not ac_section:
        return False
    match = re.search(
        r"^\s*(?:lowpowermode|powermode)\s+(\d+)\s*$",
        ac_section,
        re.MULTILINE,
    )
...
def validate_environment(snapshot: dict[str, object]) -> None:
    battery = str(snapshot["battery"])
    power = str(snapshot["power_settings"])
    thermal = str(snapshot["thermal"])
    memory = str(snapshot["memory_pressure"])
    if "AC Power" not in battery:
        raise RuntimeError("benchmark host is not on AC power")
    if not ac_power_mode_disabled(power):
        raise RuntimeError("benchmark host has low-power mode enabled or unbound")
    if "No thermal warning level has been recorded" not in thermal:
        raise RuntimeError("benchmark host thermal state is not nominal")
    if "System-wide memory free percentage:" not in memory:
        raise RuntimeError("benchmark host memory pressure identity is unavailable")
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="disabled",
        help=(
            "disabled is the only timing mode; enabled is a separate "
            "allocation-only capture whose timing fields cannot rank"
        ),
    )
    parser.add_argument(
        "--allow-noncomparable",
...
def timing_build_observation(
    ...
    expected_allocation = (
        "true" if allocation_observation == "enabled" else "false"
    )
    if build["allocation_observation"] != expected_allocation:
        return None, (
            "allocation observation build identity differs: "
            f"{build['allocation_observation']} != {expected_allocation}"
        )
    if build["callback_observer"] != "preallocated_atomic_slots_sharded_completion":
        return None, "timing callback observer is not the sharded preallocated slot design"
...
def run_attempt(
    ...
    try:
        ...
        spans = None
    trace_directory.cleanup()
    build, build_error = timing_build_observation(
        completed.stdout, spans, allocation_observation
    )
    try:
        corpus = parse_json_record(completed.stdout, CORPUS_PREFIX)
        terminals = parse_json_record(completed.stdout, TERMINALS_PREFIX)
        json_record_error = None
    except (ValueError, json.JSONDecodeError) as error:
        corpus = None
        terminals = None
        json_record_error = str(error)
    if (
        len(results) != 1
        or len(windows) != 1
        or len(resources) != 1
        or build_error is not None
        or json_record_error is not None
    ):
        return failure_attempt(
            side=side,
            phase=phase,
            ...
            detail=(
                ...
                f"json={json_record_error or 'valid'}"
            ),
            output=completed.stdout,
        )
    ...
    if evidence_error is None and wall_window_ns <= 0:
        ...
        and allocation_observation == "enabled"
        and (allocation_calls <= 0 or allocated_bytes <= 0)
    ):
        evidence_error = "enabled target allocation observation is not positive"
    elif (
        evidence_error is None
        and allocation_observation == "disabled"
        and (allocation_calls != 0 or allocated_bytes != 0)
    ):
        evidence_error = "timing binary unexpectedly emitted allocation counts"
        ...
        evidence_error = corpus_observation_error(corpus, expected_accepted)
    if evidence_error is None:
        evidence_error = terminal_record_error(
            terminals,
            callback_duplicates=callback_duplicates,
            relay_ok=relay_ok,
            relay_duplicate_ok=relay_duplicate_ok,
    ...
    if evidence_error is None and clock_delta_ns < -clock_tolerance_ns:
        evidence_error = (
    ...
    return {
        ...
        "build": build,
        "allocation_observation": allocation_observation,
        "corpus": corpus,
        "terminal_observation": terminals,
...
def aggregate_side(
    ...
    relay_unknown_parents = sum(
        int(attempt["relay_unknown_parents"]) for attempt in attempts
    )
    ...
    return {
        ...
        "terminal_observations": [
            attempt["terminal_observation"] for attempt in attempts
        ],
        "peak_rss_bytes": peak_rss_bytes,
        "reorg_latency_ns": reorg_latency_ns,
        "reorg_overlap_callbacks": reorg_overlap_callbacks,
...
def main() -> None:
    ...
    baseline_consensus_dependencies = consensus_dependency_identity(
        baseline_root, args.baseline_build_features
    )
    candidate_consensus_dependencies = consensus_dependency_identity(
        candidate_root, args.candidate_build_features
    )
    if (
        baseline_consensus_dependencies["locked_packages"]
        != candidate_consensus_dependencies["locked_packages"]
        or baseline_consensus_dependencies["enabled_features"]
        != candidate_consensus_dependencies["enabled_features"]
    ):
        raise RuntimeError(
            "baseline and candidate CKB-VM package or feature identities differ"
        )
    ...
    validate_environment(environment_start)
    record: dict[str, object] = {
        ...
        "consensus_boundary": {
            "rule": "CKB_VM_INSTRUCTIONS_CYCLES_SCRIPT_RESULT_AND_HARDFORK_MUST_BE_EXACT",
            "baseline_dependency_identity": baseline_consensus_dependencies,
            "candidate_dependency_identity": candidate_consensus_dependencies,
            "ckb_vm_package_and_enabled_features_equal": True,
            "workload_identity": (
                "EVERY_ATTEMPT_REQUIRES_EQUAL_TRANSACTION_BYTES_HASHES_CYCLES_AND_RUNTIME_CONSENSUS_DIGEST"
            ),
            "scope": "WORKLOAD_BOUND_COMPARABILITY_NOT_A_GENERAL_VM_EQUIVALENCE_PROOF",
        },
        "environment": {
        ...
        "ranking_boundary": (
            "TIMING_CPU_P99_RSS_ENABLED"
            if args.allocation_observation == "disabled"
            else "ALLOCATION_CALLS_AND_BYTES_ONLY_TIMING_FIELDS_FORBIDDEN"
        ),
        "scenarios": scenarios,
        "attempts": [],
        "measurements": [],
    ...
    for scenario in scenarios:
        ...
        pilot_failures: list[dict[str, object]] = []
        pilot_successes: list[dict[str, object]] = []
        for side, binary, root in (
            ...
            if attempt["outcome"] == "failure":
                pilot_failures.append(attempt)
            else:
                pilot_successes.append(attempt)
        if pilot_failures:
            summaries[key] = {
                ...
                "failed_sides": [
                    ...
                    for attempt in pilot_failures
                ],
            }
            record["summary"] = summaries
            write_checkpoint(args.output, record)
            continue
        if len(pilot_successes) != 2 or paired_corpus_error(
            pilot_successes[0]["corpus"], pilot_successes[1]["corpus"]
        ) is not None:
            summaries[key] = {
                "status": "non_comparable",
                "reason": "pilot_corpus_mismatch",
            }
            record["summary"] = summaries
            write_checkpoint(args.output, record)
            continue
        expected_corpus = pilot_successes[0]["corpus"]
        record["corpus_identities"][key] = expected_corpus
        write_checkpoint(args.output, record)
        ...
        baseline_unknown_parent_counts: list[int] = []
        candidate_unknown_parent_counts: list[int] = []
        ...
        for run_index in range(args.runs):
            ...
            for replicate_index in range(args.replicates_per_sample):
                ...
                for side, binary, root in order:
                    ...
                    if (
                        ...
                        and attempt["corpus"] != expected_corpus
                    ):
                        attempt["outcome"] = "failure"
                        attempt["category"] = "corpus_drift"
                        attempt["detail"] = (
                            "benchmark corpus changed after the paired pilot"
                        )
                    checkpoint(attempt)
                    cool(args.cooldown_seconds)
                    if attempt["outcome"] == "failure":
            ...
            if args.allocation_observation == "enabled":
            ...
            baseline_unknown_parent_counts.append(
                int(baseline_sample["relay_unknown_parents"])
            )
            candidate_unknown_parent_counts.append(
                int(candidate_sample["relay_unknown_parents"])
            )
        ...
        summaries[key] = {
            ...
            "median_candidate_over_baseline_allocation_calls": (
                statistics.median(paired_allocation_call_ratios)
                if paired_allocation_call_ratios
                else None
            ),
            "median_candidate_over_baseline_allocated_bytes": (
                statistics.median(paired_allocation_byte_ratios)
                if paired_allocation_byte_ratios
                else None
            ),
            ...
            "baseline_median_unknown_parent_events": statistics.median(
                baseline_unknown_parent_counts
            ),
            "candidate_median_unknown_parent_events": statistics.median(
                candidate_unknown_parent_counts
            ),
        ...
        if args.allocation_observation == "enabled":
            summaries[key] = {
                "status": "allocation_observation",
                "ranking_boundary": "TIMING_CPU_P99_RSS_FORBIDDEN",
                "replicates_per_sample": args.replicates_per_sample,
                "candidate_over_baseline_allocation_call_ratios": (
                    paired_allocation_call_ratios
                ),
                "candidate_over_baseline_allocated_byte_ratios": (
    ...
    ratios = [
        ...
        if args.allocation_observation == "disabled"
        and summary["status"] == "comparable"
    ]
    summaries["aggregate"] = {
        "mode": (
            "timing"
            if args.allocation_observation == "disabled"
            else "allocation_observation"
        ),
    ...
    validate_environment(environment_end)
    record["environment"]["end"] = environment_end
    record["summary"] = summaries
    write_checkpoint(args.output, record)
