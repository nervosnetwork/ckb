...
def run_attempt(
    ...
    with tempfile.TemporaryDirectory(prefix="ckb-txpool-span-") as temporary:
        ...
        target_contract = comparison_contract if side == "candidate" else PROTOCOL_CONTRACT
        environment["TX_POOL_BENCH_COMPARISON_CONTRACT"] = target_contract
        try:
            completed = run_process(
                command,
                cwd=root,
    ...
    try:
        ...
        build, error = timing_build_observation(completed.stdout, spans, allocation_observation, target_contract)
        if error is not None or build is None:
            raise ValueError(error)
        if comparison_contract == RBF_ABLATION_CONTRACT and (
            scenario["name"] != "rbf_pairs"
            or (side == "candidate" and build["adapter"] != "bounded_remote_batch")
        ):
            raise ValueError("RBF notice ablation requires RBF and a bounded-batch candidate")
        observed = {
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--comparison-contract", choices=(PROTOCOL_CONTRACT, RBF_ABLATION_CONTRACT), default=PROTOCOL_CONTRACT)
    ...
    if args.comparison_contract != PROTOCOL_CONTRACT and (
        args.comparison != "ab" or args.allocation_observation != "disabled"
        or any(parse_scenario(value)["name"] != "rbf_pairs" for value in args.scenario)
    ):
        parser.error("RBF contract diagnostics require an uninstrumented RBF-only A/B comparison")
...
def run_scenario(
    ...
    if args.comparison_contract != PROTOCOL_CONTRACT:
        summary = record["summary"][key]
        summary["production_ranking_permitted"] = False
        summary["comparison_contract"] = args.comparison_contract
        summary["estimand"] = "Candidate / baseline under the declared RBF victim-notice contract difference; includes all other source differences, changed obligations and scheduling. This alone cannot isolate notification cost or pure implementation CPU."
        if summary["status"] == "comparable":
            summary["status"] = "contract_diagnostic"
    write_checkpoint(output, record)
...
def main() -> None:
    ...
    accepted_status = ("calibrated" if args.calibration_only else "contract_diagnostic"
                       if args.comparison_contract != PROTOCOL_CONTRACT else "aa_equivalent"
