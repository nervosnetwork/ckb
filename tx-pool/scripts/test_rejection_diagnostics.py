...
class RejectionEvidenceTests(unittest.TestCase):
    ...
    def test_timeout_retains_flushed_output_and_cleans_the_owned_process(self):
        rejection_output = encode(fixture()[:2])
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            binary = root / "stuck-workload"
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as listener:
                address = str(root / "witness.sock")
                ...
                binary.write_text(
                    ...
                    "witness = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)\n"
                    f"witness.connect({address!r})\n"
                    "print('flushed stdout before timeout', flush=True)\n"
                    f"print({rejection_output!r}, file=sys.stderr, flush=True)\n"
                    "witness.sendall(b'ready')\n"
                    "while True: signal.pause()\n"
                )
                binary.chmod(0o700)
                attempt = benchmark.run_attempt(
                    benchmark.binary_record(binary), root,
                    benchmark.parse_scenario("rbf_pairs,2,2,1,2"),
                    "baseline", "timeout-capture", 2, "disabled",
                )
                with listener.accept()[0] as witness:
                    witness.settimeout(5)
                    # The child acknowledges only after both streams flush;
                    # EOF proves the owned process closed its lifetime witness.
                    with witness.makefile("rb") as stream:
                        self.assertEqual(stream.read(), b"ready")
                self.assertEqual(attempt["outcome"], "failure")
                self.assertEqual(attempt["category"], "runner_timeout")
                self.assertIn("flushed stdout before timeout", attempt["output"])
                self.assertIn(rejection_output, attempt["output"])
                self.assertNotIn("RESOURCE_RESULT", attempt["output"])
