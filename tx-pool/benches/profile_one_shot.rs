...
#[cfg_attr(
    feature = "cross-version-legacy-bench-adapter",
    allow(
        clippy::needless_update,
        reason = "The shared benchmark sets every legacy field; current-only fields retain their defaults."
    )
)]
...
fn run() -> BenchResult<()> {
    ...
    println!(
        "BENCH_BUILD profiling={} allocation_observation={} callback_observer=preallocated_atomic_slots_sharded_completion adapter={} debug_assertions={} measurement_window=terminal_completion_v3 comparison_contract=protocol",
        cfg!(feature = "profiling"),
