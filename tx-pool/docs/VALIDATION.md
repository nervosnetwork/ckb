...
| [`mutation-result-lock.json`](../mutation-result-lock.json) | Generated portable result projection binding the candidate lock and exact raw-outcome digests to one reconciled outcome per unique candidate. It records execution and replay counts and rejects a repeated identity whose outcome changes across runs. | `check_mutation_matrix.py` | Generate only with one or more `--verify-outcomes` inputs plus `--write-result-lock`. Never edit candidate identities, outcomes, counts or artifact digests by hand. `accepted: false` remains a diagnostic checkpoint rather than V1 evidence. |
