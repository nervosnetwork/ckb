...
| [`mutation-acceptance-lock.json`](../mutation-acceptance-lock.json) | Generated V1 row-level projection binding every selected cargo-mutants candidate to one semantic obligation, one production owner, generated invariant/falsifier evidence and the complete library Nextest universe. It also freezes input/tool hashes and the exact candidate/config/command digests. | `generate_mutation_matrix.py` | Generate only with `--write-lock` from a clean reviewed semantic checkpoint. Never edit rows, paths, functions, tests, counts, commands or digests by hand; ordinary generator runs reject drift. |
