...
| `TP-DEP-001` Causal dependency graph | `tx-pool/src/component/pre_pool/wait.rs`<br>`tx-pool/src/component/pre_pool/lifecycle.rs`<br>`tx-pool/src/resolved_tx.rs`<br>`tx-pool/src/component/links.rs`<br>`tx-pool/src/pool_cell.rs`<br>`tx-pool/src/component/tx_selector.rs`<br>`tx-pool/src/authority/chain.rs`<br>`tx-pool/src/authority/dependency.rs`<br>`tx-pool/src/authority/work.rs`<br>`tx-pool/src/authority/plan.rs`<br>`tx-pool/src/authority/effect.rs`<br>`tx-pool/src/authority/runtime.rs` | Raw, resolved and accepted exact dependency keys—including the complete direct missing-cell frontier, headers and expanded dep-group members—remain one canonical primary fact: availability wakes children and definitive loss invalidates them atomically while by-parent is only a derived projection. A Remote transition to Waiting(Missing) commits one canonical transaction-parent request in the same Apply; trusted sources may wait only on a missing parent already owned by the pre-pool and manufacture no network work. Because that disposition is source policy, a Remote-to-Proposal promotion cannot reuse an external-parent Waiting observation: it returns to the existing Resolve level and is adjudicated once under trusted policy. | A cell provider reporting only its first miss, duplicate missing outputs from one parent, late-discovered parents, effect backpressure, conditional reader/spender cycles, reversed arrival order, stale resolved children, parent replacement or source promotion of a Remote external-parent waiter must not strand a child, make RelayV3 reject an unrequested parent, hot-loop resolution, lose its wake edge or let a template commit an invalid order. | T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13 | - Are input, cell-dep, header-dep and expanded dep-group roles intentionally distinguished?<br>- Does parent success/failure update reverse edges, accounting and child location in one transition?<br>- Can a source transition reuse its current wait only when the destination policy admits the same missing evidence, with Remote-to-Proposal otherwise returning to Resolve?<br>- Are cascade size and maintenance work explicitly bounded? | Use bounded indexed parent/child buckets and maintenance slices; never poll all waiting transactions or scan the pool for dependents. |
...
| `TP-PERSIST-001` Coherent persistence recovery point | `tx-pool/src/service.rs`<br>`tx-pool/src/persisted.rs`<br>`tx-pool/src/component/pre_pool/recovery.rs`<br>`tx-pool/src/component/pool_map.rs`<br>`tx-pool/src/process/reorg.rs`<br>`tx-pool/src/authority/read.rs`<br>`tx-pool/src/authority/query.rs`<br>`tx-pool/src/authority/runtime.rs` | Under one TxPool-read then PrePoolKernel snapshot boundary, persistence copies causal-parent-first accepted entries plus every retained/leased recovery-source payload into a bounded v2 envelope, releases authority locks, and serializes atomic writers without draining live state; startup also accepts the legacy v1 vector and revalidates every transaction. | Save racing detached replay/clear, an active recovery lease, malformed or oversized disk data, writer failure or expanded dep-group ordering must not persist an ownership gap, allocate from an unbounded length, lose the previous atomic file, drain the live pool or serialize children before required parents. | T1, T2, T3, T4, T6, T7, T8, T9, T10, T11, T12, T13 | - Does save use only the universal TxPool-read then kernel order and include accepted xor every recovery owner at one linearization point?<br>- Is PoolMap the accepted ordering authority, including expanded dependencies, while recovery metadata preserves parent-first session order?<br>- Are file bytes/counts bounded before allocation, writes temp-sync-rename atomic, and all startup payloads revalidated rather than trusted? | Shutdown-only clone/sort work stays off admission paths; do not add a continuously maintained persistence projection. |
| `TP-QUERY-001` Coherent bounded public projections | `tx-pool/src/service/controller.rs`<br>`tx-pool/src/service/dispatch.rs`<br>`tx-pool/src/authority/read.rs`<br>`tx-pool/src/authority/query.rs`<br>`tx-pool/src/authority/runtime.rs` | Every tx-pool status, payload, aggregate, graph detail and paired chain fact is compiled from one borrowed authority cut into an owned receipt. Replacement history remains private and falls through to recent-reject semantics; accepted Gap remains RPC Pending; compact and live-cell storage lookup, fee computation, sorting and payload destruction occur after the authority guard is released. | A clear, reorg, replacement, short-ID lookup, large pool query or slow database read must not splice owner generations, expose history as live Pending, fabricate verification facts, hold the authority during external storage work, turn a cold query into O(pool squared) work or make a projection a second membership authority. | T1, T2, T3, T4, T6, T7, T8, T9, T10, T12 | - Does each service response come from one authority/snapshot cut rather than several independently locked reads?<br>- Are raw hashes authoritative while proposal short IDs use only the unique collision-checked index?<br>- Are database access, topological sorting, fee simulation and large object destruction outside the authority guard?<br>- Do single-item queries use indexed or bounded graph work, and do full-vector queries perform at most one ordered population traversal?<br>- Are ReplacementHistory, Pending, Gap, Proposed, cycles, fees and timestamps mapped exactly to the established RPC compatibility surface? | Single-item owner and graph lookups are indexed or explicitly bounded; full-pool DTOs use one O(pool) traversal and bounded allocation, while database I/O, persistence ordering and fee simulation execute after the authority read guard opens. |
| `TP-DEFECT-001` Rust-native failure and persistence boundary | `tx-pool/src/component/pre_pool/runtime.rs`<br>`tx-pool/src/component/pre_pool/lifecycle.rs`<br>`tx-pool/src/process/mod.rs`<br>`tx-pool/src/process/submit/mod.rs`<br>`tx-pool/src/metrics.rs`<br>`tx-pool/src/service/builder.rs`<br>`tx-pool/src/service/effects.rs`<br>`tx-pool/src/service/stages/resolve.rs`<br>`tx-pool/src/service/dispatch.rs`<br>`tx-pool/src/service/stages/verify.rs`<br>`sync/src/relayer/mod.rs`<br>`util/metrics/src/lib.rs` | Static ownership and private constructors make invalid authority states unconstructable; every legal malformed, policy, capacity, duplicate, stale, resource, endpoint and rebuildable-projection outcome remains typed and locally contained. Controlled stop without persistence is reserved for a pre-Apply fault that proves authoritative integrity is already lost and excludes legal/hostile input plus every supported interleaving at its call site; it is never converted to an RPC/peer outcome. Panic plus catch_unwind is not authority, worker, retry or rollback control flow. Low-cardinality counters observe the existing typed-fault, worker-exit, handler-unwind and effect-publisher boundaries but never select them. | A reproducible legal transaction, stale lease, foreign endpoint failure/hang, relay saturation or rebuildable projection drift must not panic, stop the service or reach a structural fault; a proven authoritative-integrity loss must not be converted to success/rejection, restarted into an unknown generation or written to persistence. | T1, T2, T3, T4, T6, T7, T8, T9, T10, T11, T12, T13 | - Can every peer-selectable legal outcome be traced to a typed result before mutation without assert, expect, unwrap, panic or unreachable?<br>- Does the type system prevent mutation between Plan and single-consumption total Apply?<br>- Has panic plus catch_unwind been removed from internal job, worker, publisher and authority protocols?<br>- Are genuinely foreign callbacks/endpoints isolated without selecting tx-pool state, retry or rollback semantics?<br>- Does every controlled-stop mapping carry a call-site proof that authoritative integrity is already lost and that legal/hostile input, stale work, resource pressure, external failure and supported interleavings cannot reach it?<br>- Are rebuildable projection failures discarded, rebuilt or safely degraded without stopping unrelated tx-pool work? | Proof-carrying plans and typed results are zero-cost ownership encodings on the healthy path; local projection recovery reuses level-triggered source state and adds no recovery mutex, quarantine lookup, spare generation, unwind-driven retry or restart protocol. |
...
| `TP-TEMPLATE-001` Block-template liveness and priority | `tx-pool/src/block_assembler`<br>`tx-pool/src/authority/packing.rs`<br>`tx-pool/src/authority/template.rs`<br>`tx-pool/src/authority/template_driver.rs`<br>`tx-pool/src/pool.rs`<br>`tx-pool/src/process/reorg.rs` | CurrentTemplate is the sole authority for published revision and reset generation; level-triggered convergence owns only desired and covered source cuts. One ordered reset/full lane and concurrent optimistic uncle, proposal and transaction lanes construct outside authority and output locks. Full wins over partial-only revisions but cannot cross an exact requested reset or a chain transition committed after its Plan; reset publishes the exact move-only generation it received, including skipped generations. A final liveness mismatch degrades to the deterministic checked transaction subset for that source cut without mutating pool authority, retry-spinning or exiting the template lane. Transaction packing is a pure, short-lived consumer of one coherent authority receipt: it preserves exact byte/cycle limits, ancestor-inclusive CPFP ordering, dynamic descendant re-scoring, parent-first inclusion, bounded conditional-cycle shedding and the bounded consecutive-failure policy without creating another graph or owner. Candidate-uncle preparation carries an opaque exact source receipt; insertions advance that source, while committed pruning proven selection-neutral by the paired chain cut does not manufacture redundant source work. External template scripts are observational, own at most one live child per configured command and terminate that child on timeout; HTTP notification work remains cancellable and outside authority locks. | Detached uncle proposals, stale Gap status, an oversized or cyclic CPFP shape, candidate-source counter exhaustion, an update_full/partial race, skipped reset generation, chain transition between Plan and Apply, stale partial acknowledgement or finally-unresolvable selected transaction must not make a valid transaction RPC-pending but forever absent from normal get_block_template mining, exceed consensus limits, publish old-chain content, publish an invalid dependency order, mutate before a typed failure, retry-loop, stop template service, or serialize independent partial work behind full calculation. A hung notification script must not turn repeated template updates into unbounded child processes, and a hung HTTP endpoint must not retain tx-pool authority past its configured timeout. | T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13 | - Does the co-located revision/reset epoch preserve full-over-partial priority and reset-over-stale-full authority while re-dirtying uncle/proposal/transaction generations after replacement?<br>- Does a full rebuild derive detached uncles from the candidate authority instead of copying the reset template's transient blank projection?<br>- Can uncle proposal filtering exclude the sole proposal path of a recovered transaction?<br>- Are Gap/Pending/Proposed transitions reflected to assembler selection?<br>- Does the pure packer consume only one coherent accepted receipt, preserve exact CPFP/limit policy and reuse the bounded conditional-ordering kernel rather than storing another graph?<br>- Does candidate-uncle state expose only opaque source receipts, with typed mutation failure and selection-neutral stale pruning?<br>- Do uncle/proposal/transaction calculations remain concurrent with a waiting full rebuild and publish only through version checks?<br>- Does Apply validate the move-only build's exact chain source against the authoritative source while CurrentTemplate remains the only published revision/reset owner?<br>- Does a final liveness miss publish only the checked subset without changing membership, retry-spinning or exiting a worker?<br>- Can any configured notification script accumulate live owned children in proportion to template update rate?<br>- Does HTTP notification remain cancellable and bounded by visible trusted configuration rather than transaction ownership? | Keep pure packing and graph work outside authority locks, optimistic CAS partial updates concurrent with full/reset construction, bounded descendant/SCC work and the 4,000-failure safeguard; do not add a persistent template graph, serialize every delta behind full rebuild or remove bounds without profiling and fixed-binary A/B evidence. |
| `TP-IDENTITY-001` Full transaction and witness identity | `tx-pool/src/process/submit/mod.rs`<br>`tx-pool/src/component/pre_pool/mod.rs`<br>`tx-pool/src/component/pool_map.rs`<br>`tx-pool/src/authority/resolver.rs`<br>`tx-pool/src/authority/runtime.rs` | Ownership and duplicate boundaries use full raw hashes, proposal short IDs remain non-authoritative indexes, and verification-cache proofs are keyed only by the exact witness hash plus ScriptVerificationRules through TxVerificationCacheKey. A prepared tx-pool verification request must bind its own exact cache lookup before execution; callers cannot provide a separately sampled nearby Completed value. | Short-ID collisions or same-raw/different-witness variants must not alias accepted/cache/history ownership, obtain a false duplicate success or reuse an invalid verification proof during normal admission or reorg. A cached result for request A must not be pairable with request B even when their surrounding snapshot and policy are identical. | T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13 | - Is a short ID used only where collision-aware lookup semantics are explicit?<br>- Can any cache call construct a key from raw hash or arbitrary bytes?<br>- Does reorg recovery query the exact transaction witness variant?<br>- Can only a cache-bound request execute, making the request identity and lookup result constructive evidence rather than a call-site convention? | Use compact full hashes/typed cache keys without retaining packed backing; collision handling remains indexed and bounded. |
...
- `uak_matching_resolution_completion_requeues_across_a_chain_view_change` (T1, T2, T3, T6, T9, T11)
...
- `uak_verify_checkout_requeues_resolution_from_an_old_chain_view_before_vm` (T1, T2, T3, T6, T8, T9, T11)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(parent_commit_before_wait|parent_loss|successive_expanded_parent_losses|terminalized_superseded_parent|dependency_epochs|dep_reader|selected_reader|conditional_cycle|dep_group|unknown_outpoint|uak_remote_missing|uak_proposal_promotion_reclassifies_remote_missing_wait|uak_direct_parent_acceptance|uak_chain_tip_not_revision|uak_coupled_membership_requires|uak_template_orders_selected|uak_template_sheds)/)'`
...
- `uak_proposal_promotion_reclassifies_remote_missing_wait_under_trusted_policy` (T1, T2, T3, T4, T7, T8, T11, T13)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --lib -E 'test(/(owner_fairness_and_active_caps|expiry_batch_is_bounded|worker_|zero_verify_worker|stage_completion_checks_out_same_lane|checked_out_continuation|verify_continuation_uses|uak_stale_dependency_head|uak_stale_remote_cycle_rejection|uak_remote_verify_failure|runtime_shared_compute_gate|uak_maintenance_driver|uak_idle_maintenance)/)'`
...
- `runtime_shared_compute_gate_bounds_mixed_retained_and_direct_work` (T1, T2, T3, T4, T7, T8, T10, T11, T13)
- `service_cancellation_is_scoped_under_process_exit` (T1, T4, T7)
- `uak_idle_maintenance_driver_waits_instead_of_spinning` (T1, T2, T3, T4, T7, T8, T10, T11, T13)
- `uak_maintenance_driver_fairly_drains_every_preexisting_level` (T1, T2, T3, T4, T7, T8, T10, T11, T13)
- `uak_released_allocation_failure_backs_off_before_recompute` (T1, T2, T3, T7, T8, T10, T11, T13)
...
- `uak_local_non_remote_preaccepted_removal_does_not_release_relay_state` (T1, T2, T3, T4, T7, T10, T11)
...
- `uak_effect_compiler_keeps_rejection_owner_and_ingress_attribution_typed` (T1, T2, T4, T7, T8, T10)
...
- `uak_mixed_ready_settlement_preserves_effect_headroom_by_source_control` (T1, T2, T3, T7, T8, T10, T11, T13)
- `uak_ordinary_relay_saturation_reconciles_and_retains_later_results` (T3, T7, T8, T10, T11, T13)
...
- `uak_publisher_relay_disconnect_disposes_and_drains_the_authority_head` (T1, T3, T7, T10, T11)
- `uak_relay_disconnect_opens_a_stable_external_circuit` (T3, T7, T10, T11)
- `uak_relay_mailbox_overflow_orders_reset_before_the_current_result` (T3, T4, T7, T8, T10, T11, T13)
- `uak_relay_parent_rebuild_pages_the_authoritative_missing_level` (T1, T2, T3, T4, T7, T8, T10, T11, T13)
- `uak_remote_expiry_effect_backpressure_is_zero_mutation` (T1, T2, T3, T7, T8, T10, T11, T13)
- `uak_required_parent_detail_follows_reset_after_relay_saturation` (T3, T4, T7, T8, T10, T11)
- `uak_retained_batch_resumes_at_its_first_unprocessed_endpoint` (T1, T2, T3, T7, T10, T11)
- `uak_retained_later_endpoint_does_not_replay_completed_callback_cursor` (T1, T2, T3, T7, T10, T11)
- `uak_unregistered_callback_does_not_enter_the_blocking_boundary` (T3, T7, T8, T10)
...
- `uak_chain_boundary_retains_ordered_backpressure_without_open_plan_errors` (T1, T2, T7, T9, T10, T12, T13)
...
- `uak_persistence_receipt_is_owned_and_mutation_independent` (T1, T2, T3, T4, T8, T9, T10, T11, T12, T13)

#### `TP-QUERY-001` — Coherent bounded public projections

Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(uak_owned_transaction_query|uak_owned_pool_queries|uak_resolved_preaccepted_query|uak_live_cell_receipt|uak_compact_receipt|uak_fee_receipt)/)'`

Rust evidence:

- `uak_compact_receipt_releases_authority_before_storage_lookup` (T1, T2, T8, T9, T10, T12)
- `uak_fee_receipt_is_owned_and_mutation_independent` (T1, T2, T6, T8, T9, T12)
- `uak_live_cell_receipt_releases_authority_before_storage_lookup` (T1, T2, T6, T8, T9, T10, T12)
- `uak_owned_pool_queries_share_one_status_and_aggregate_cut` (T1, T2, T3, T6, T8, T9, T12)
- `uak_owned_transaction_query_hides_replacement_history_and_reports_minimum_fee` (T1, T2, T3, T6, T7, T8, T12)
- `uak_resolved_preaccepted_query_uses_current_proposal_window` (T1, T2, T4, T6, T9, T12)
...
- `runtime_stale_plan_disposition_depends_on_the_producer_boundary` (T1, T2, T3, T7, T9, T10, T11, T13)
- `uak_allocation_failure_discards_result_without_retaining_compute_capability` (T1, T2, T3, T7, T8, T10, T11, T13)
- `uak_duplicate_inputs_are_a_malformed_outcome_not_an_authority_fault` (T1, T2, T4, T7, T10, T11)
- `uak_topology_derived_timeout_does_not_forbid_persistence` (T1, T2, T3, T7, T9, T10, T11, T12, T13)
- `uak_topology_forbids_persistence_only_after_authority_integrity_loss` (T1, T2, T3, T7, T9, T10, T11, T12, T13)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --features internal --lib -E 'test(/(reorg_demotes_stale_gap|pending_proposals_filter|full_reset_and_partial_priority_use_template_owned_tokens|full_rebuild_derives_uncles_from_candidate_authority|reorg_refresh_recovers_when_blank_reset_precedes_candidate_retention|full_rebuild_reissues_every_optimistic_delta|one_configured_script_owns_at_most_one_live_process_slot|selected_reader|conditional_cycle|uak_template|uak_accepted_timestamp|uak_chain_observation_reconciles|uak_runtime_chain_boundary_reconciles|candidate_uncle_receipt|candidate_uncle_source)/)'`

Rust evidence:

- `candidate_uncle_receipt_is_exact_and_committed_stale_prune_is_version_neutral` (T2, T3, T8, T9, T10, T12, T13)
- `candidate_uncle_source_version_exhaustion_is_typed_and_mutation_free` (T3, T7, T8, T10, T12, T13)
...
- `uak_template_cycle_shedding_preserves_descendant_aware_strength` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_dependency_budget_cannot_censor_later_independent_work` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_driver_degrades_to_the_finally_live_subset_without_lane_failure` (T3, T7, T8, T9, T10, T12, T13)
- `uak_template_driver_full_priority_and_partial_occ_use_one_output_revision` (T3, T9, T10, T12, T13)
- `uak_template_driver_publishes_the_exact_latest_reset_generation` (T2, T3, T9, T10, T12, T13)
- `uak_template_driver_rechecks_candidate_source_and_filters_proposal_conflicts` (T2, T3, T9, T10, T12, T13)
- `uak_template_driver_reproposes_recovered_gap_and_commits_after_proposal` (T4, T9, T10, T12, T13)
- `uak_template_drivers_cancel_cleanly_without_idle_publication` (T7, T8, T10, T13)
- `uak_template_packer_aggregates_multi_parent_descendant_adjustments` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_packer_bounds_long_conditional_scc_fallback` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_packer_bounds_non_fitting_work_without_changing_the_policy` (T3, T8, T10, T12, T13)
- `uak_template_packer_requires_proposed_ancestors_and_orders_conditional_edges` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_packer_rescores_descendants_after_shared_parent_selection` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_packer_selects_an_exact_fit_cpfp_package_parent_first` (T3, T4, T8, T10, T11, T12, T13)
- `uak_template_read_receipt_shares_order_and_complete_resolved_payload` (T1, T2, T3, T4, T9, T10, T12, T13)
- `uak_template_rebuild_failure_retains_the_last_projection_and_lane` (T2, T3, T7, T9, T10, T12, T13)
- `uak_template_receipts_repair_overwrite_and_delayed_delta` (T9, T10, T12, T13)
- `uak_template_reset_counter_exhaustion_is_typed_and_mutation_free` (T3, T7, T8, T10, T11, T12, T13)
- `uak_template_source_probe_skips_irrelevant_population_captures` (T2, T3, T8, T9, T10, T12, T13)
...
Minimum command: `cargo nextest run -p ckb-tx-pool --lib -E 'test(/(short_id_collision|full_hash_lookup|proposal_witness_variant|trusted_conflict_resubmission|verification_cache_isolated|reorg_recovery_reads_cache|uak_verification_cache_lookup)/)'`
...
- `uak_verification_cache_lookup_cannot_substitute_a_nearby_request` (T1, T2, T3, T5, T6, T7, T11)
