...
| Generated legal operations preserve complete populations and reject a stale prior cut after every step | [state_sequences.rs](../src/authority/tests/state_sequences.rs); fixed seeds, explicit replay and failure reduction, with [scope and commands](MAINTENANCE.md#repeat-composed-workloads-and-replay-sequences) |
| Repeated pressure, replacement, dependency wake, attach/detach and clear release resources in one service generation | [stability.rs](../src/authority/tests/stability.rs); exact callback/relay outcomes, polled publication barrier, same-peer retry, observed payload release and empty owner/index/queue/account/active/outbox checkpoints; extended RSS/latency observations remain diagnostic |
...
| Committed candidate diagnostics, transient-status exclusion, and same-variant RBF candidate/victim distinction | [notice.rs](../src/authority/tests/notice.rs); [execution.rs](../src/authority/tests/execution.rs) checks real peer pressure retains the victim and same-peer retry succeeds |
