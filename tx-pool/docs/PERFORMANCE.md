...
constraint, repair a topology defect or close the open architecture-class
research target.
...
The static frontier over the open architecture class is not closed and remains
research `OPEN_FROZEN`; no finite result establishes an open-class singleton.
Candidate ranking is currently unauthorized for a different, concrete reason:
terminal correctness has blocking findings and the next finite candidate/
develop generation has not been frozen. The historical finite P3 matrix is
invalid and contributes no admissible product-performance outcome.

Future measurements may compare only candidates admitted by the terminal
correctness and result-before-frozen finite-candidate gates. Acceptance
measurements must bind the final source,
...
The required cost dimensions in `ARCHITECTURE.md` are normative. The table
below describes costs in the current implementation and remains diagnostic
evidence; it does not prove that the current stage/task topology is minimal. The
...
outside its scope. Production-bound operation-count properties and the
structural contract checker bind the equation to the actual batch, stage and
settlement owners so later topology changes cannot silently preserve obsolete
prose.
...
2. **Direct verified commit - measured and rejected.** The experiment reused
   ...
   This result names the retired retained-compute shortcut: it kept a compute
   permit through final membership and bypassed Ready. It does not describe the
   later lock migration of the already synchronous Local Direct path. That
   migration preserves the validation/Ready topology and has no performance
   claim until its complete true-shard source is independently measured.
   Complete production Direct finalization now uses the shared outer guard:
   accepted/RBF/capacity outcomes commit through exact owner and projection
   cuts, while duplicate/rejection outcomes hold exact read support through
   sole-journal activation. The maintained workload matrix does not isolate
   these shapes, so this lock migration likewise carries no empirical speedup
   claim.
...
#### Historical M3.6 topology decision

At that source identity, the bounded comparison selected the **bounded semantic
exchange** among the evaluated alternatives. The production implementation
...
historical production-bound operation-count relation gives:
...
The retained-compute shortcut is removed and Ready is retained as a necessary
pipeline concurrency boundary as well as an authority state. This is an
example where fewer transitions and lower CPU work did not imply higher
throughput; both operation evidence and paired timing were required for
disposition.
...
### 3.6 Historical pre-terminal-audit A/B status
...
not a performance claim. Its historical capture path was
`/private/tmp/txp-medium-prepf-final-p4-v6.json`, SHA-256
`96217b6e180f959fc5743fd80ebb95bbde324a34490fb6057bb3fa1f62d6a3d6`.
That temporary path is not a recovery dependency and may no longer exist; the
record remains non-admissible even if the bytes are later recovered.
...
| `tx_pool.scheduler.queue_stage_wait`, `queue_stage_hold` | Acquisition and complete lifetime of a cursor-free hidden scheduler stage. Disjoint queue stages may overlap; a waiting fairness stage prevents later queue-stage barging. |
| `tx_pool.scheduler.fairness_stage_wait`, `fairness_stage_hold` | Writer-preferred wait and exclusive lifetime for a hidden scheduler stage that owns a resolve/verify fairness cursor. |
...
The future `MEASURED_STRONGEST`/performance-Acceptance layer must cover both
ordinary and adversarial CKB shapes:
...
The next finite-candidate generation must freeze its exact membership before
observing results. Its required comparisons are:

1. the terminally accepted true-shard candidate versus a freshly frozen
   `develop` baseline for every compatible precommitted scenario;
2. an optional candidate-versus-prior-true-shard diagnostic used only to
   attribute the retained optimization value, never to select the delivery
   winner;
...
Medium or full repeated records may close only that frozen finite-candidate
performance layer; full may be run on a more suitable host, but all artifacts
must satisfy the same fingerprint contract. A noisy or
