...
impl TxPool {
    pub(crate) fn load_from_file(&self) -> Result<Vec<TransactionView>, AnyError> {
        ...
        if persisted_data_file.exists() {
            let mut file = OpenOptions::new()
                ...
                .map_err(|err| {
                    let errmsg = format!(
                        "Failed to open the tx-pool persisted data file [{:?}]: {}",
            ...
            file.read_to_end(&mut buffer).map_err(|err| {
                let errmsg = format!(
                    "Failed to read the tx-pool persisted data file [{:?}]: {}",
                    persisted_data_file, err
                );
                OtherError::new(errmsg)
            })?;
            ...
            let persisted_data = TransactionVecReader::from_slice(&buffer)
                .map_err(|err| {
                    let errmsg = format!(
                        "The tx-pool persisted data file is broken, please delete it and restart: {}",
                        err
                    );
    ...
    pub(crate) fn save_into_file(&self) -> Result<(), AnyError> {
        let mut persisted_data_file = self.config.persisted_data.clone();
        persisted_data_file.set_extension(format!("v{}", VERSION));
        ...
        file.write_all(txs.as_slice()).map_err(|err| {
            let errmsg = format!(
                "Failed to write the tx-pool persisted data into file [{:?}]: {}",
                persisted_data_file, err
            );
            OtherError::new(errmsg)
        })?;
        file.sync_all().map_err(|err| {
            let errmsg = format!(
                "Failed to sync the tx-pool persisted data file [{:?}]: {}",
                persisted_data_file, err
            );
            OtherError::new(errmsg)
        })?;
