...
/// Inputs consumed by accepted transactions at one coherent pool and chain cut.
/// Waiting and replacement history do not consume cells. The snapshot owns only
/// compact outpoints, so consumers do not retain transaction or VM payloads.
#[derive(Clone)]
pub struct TxPoolInputSnapshot {
    /// Chain tip paired with these accepted transactions.
    pub tip_hash: Byte32,
    /// Unique inputs consumed by pending, gap and proposed transactions.
    pub inputs: std::sync::Arc<HashSet<OutPoint>>,
}

impl fmt::Debug for TxPoolInputSnapshot {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("TxPoolInputSnapshot")
