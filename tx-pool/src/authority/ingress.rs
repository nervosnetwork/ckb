...
use super::{
    jobs::Resolution,
    model::{Entry, Error, FullReason, Phase, RemoteOrigin, Source},
    notice::{Class, Effect},
    store::{Plan, Store},
};
use crate::{
...
/// Validate a received transaction, then retain or replace its current owner.
/// Duplicate relay input only acknowledges that owner. A proposal may promote
/// network work, preserving its origin and any still-current resolution.
pub(super) fn prepare(
    ...
    if let Source::Remote { origin, .. } = source
        && plan.peer_banned(store, origin.peer)?
    {
    ...
    let (source, phase) = match (current.as_deref(), source) {
        (Some(old), Source::Remote { origin, .. }) => {
            let tx_hash = compact_packed(&transaction.hash());
            let result = if old.accepted().is_some() {
                TxVerificationResult::Ok {
                    original_peer: Some(origin.peer),
                    tx_hash,
                }
            } else {
                TxVerificationResult::Reject { tx_hash }
            };
            plan.notify(Effect::relay(result));
            return Ok(plan);
        }
        (Some(old), _) if old.accepted().is_some() => return Ok(plan),
        (Some(old), Source::Proposal { .. }) => {
            if matches!(old.source, Source::Recovery | Source::Local) {
                return Ok(plan);
            }
            let same_witness = old.transaction.witness_hash() == transaction.witness_hash();
            if same_witness && matches!(old.source, Source::Proposal { .. }) {
                return Ok(plan);
            }
            // Promotion changes verification policy, not successful resolution.
            // Its original view and reads must still be checked by the worker.
            let phase = match &old.phase {
                Phase::Verify(resolved) if same_witness && resolved.view == view => {
                    Phase::Verify(Arc::clone(resolved))
                }
                _ => Phase::Resolve,
            };
            (old.source.promote_remote(), phase)
        }
        (None, Source::Proposal { .. }) => (Source::Proposal { remote: None }, Phase::Resolve),
        (_, source) => (source, Phase::Resolve),
    };
