...
use super::{
    ...
    shard::{ShardReadSupport, ShardedOwnerMap, ShardedOwnerWriteCut},
    state::{
...
impl VerificationTimePolicy {
    ...
    pub(in crate::authority) fn duration(self, payload_policy: PayloadPolicy) -> Duration {
...
impl AcceptedOverlay {
    ...
    /// Exact physical support for the Accepted observations retained by this
    /// overlay. Every queried producer owns a positive-or-negative owner row;
    /// every input also owns a positive-or-negative spender row. A positive
    /// spender includes its owner row so the final fence can revalidate the
    /// exact Accepted incarnation without acquiring another shard later.
    pub(super) fn sharded_read_support(&self, entries: &ShardedOwnerMap) -> ShardReadSupport {
        let mut support = ShardReadSupport::default();
        for query in &self.queries {
            support.insert(entries.owner_shard(&query.producer));
            if query.role == CellRole::Input {
                support.insert(
                    entries
                        .layout
                        .router
                        .shard(b"membership/spender", &query.out_point),
                );
            }
        }
        for spender in self.spent_inputs.values() {
            support.insert(entries.owner_shard(&spender.key));
        }
        support
    }

    /// Recheck the exact producer/spender relation while its complete routed
    /// read support remains held. The caller may activate its already-staged
    /// effect before dropping `cut`; no relevant Accepted Apply can interpose
    /// between this check and that activation.
    pub(super) fn is_current_in_cut(
        &self,
        entries: &ShardedOwnerMap,
        cut: &ShardedOwnerWriteCut<'_>,
    ) -> bool {
        self.queries.iter().all(|query| {
            self.producer_version(query)
                == Self::current_producer_version_in_cut(entries, cut, query)
                && (query.role != CellRole::Input
                    || Self::spender_is_current_in_cut(
                        self.spent_inputs.get(&query.out_point),
                        entries,
                        cut,
                        &query.out_point,
    ...
    fn current_producer_version_in_cut(
        entries: &ShardedOwnerMap,
        cut: &ShardedOwnerWriteCut<'_>,
        query: &CellQuery,
    ) -> Option<EntryVersion> {
        let Some(OwnedTx::Accepted(entry)) = cut.owner(entries, &query.producer) else {
            return None;
        };
        let index: u32 = query.out_point.index().unpack();
        let index = usize::try_from(index).ok()?;
    ...
    fn spender_is_current_in_cut(
        expected: Option<&AcceptedOwnerObservation>,
        entries: &ShardedOwnerMap,
        cut: &ShardedOwnerWriteCut<'_>,
        out_point: &OutPoint,
    ) -> bool {
        let current = cut.membership_spender(entries, out_point);
        match expected {
            None => current.is_none(),
            Some(expected) => {
                if current != Some(&expected.key) {
                    return false;
                }
                matches!(
                    cut.owner(entries, &expected.key),
                    Some(OwnedTx::Accepted(entry)) if entry.record.version == expected.version
                )
            }
        }
    }
    ...
    pub(in crate::authority) async fn execute(
        ...
        let outcome = match verified {
            ...
            Ok(TxPoolVerificationOutcome::InitialLoadExceeded) => {
                return VerificationExecution {
                    settlement: work.rejected(Reject::Full(
            ...
            Ok(TxPoolVerificationOutcome::InitialLoadExceeded) => {
                return Ok(DirectVerificationOutcome::Rejected(
                    DirectTransactionRejection::accepted_reads(
                        ...
                        command,
                        Reject::Full(
                            "transaction exceeds the tx-pool initial program load envelope"
                                .to_owned(),
                        ),
                        location.into_view(),
                        accepted_reads,
