...
/// Result of trying to capture a population-sized query from one coherent
/// authority cut.  Scratch growth is deliberately requested by the cut that
/// will populate it; a separately sampled owner count is not a safe capacity
/// witness once ordinary shard commits may proceed under the shared outer
/// generation barrier.
pub(super) enum FullQueryCapture<T> {
    Prepared(T),
    NeedsGrow(usize),
}
...
impl FullQueryPermit {
    ...
    pub(super) fn capture_pool_ids<'permit>(
        &'permit mut self,
        view: &AuthorityReadView<'_>,
    ) -> Result<FullQueryCapture<PreparedPoolIds<'permit>>, AuthorityReadError> {
        let cut = view.full_read_cut()?;
        let observed_rows = cut.owner_count();
        if !self.begin_capture(observed_rows)? {
            return Ok(FullQueryCapture::NeedsGrow(observed_rows));
        }
        ...
        Ok(FullQueryCapture::Prepared(PreparedPoolIds {
            permit: self,
            pending_count,
            proposed_count,
        }))
    ...
    pub(super) fn capture_entry_info<'permit>(
        &'permit mut self,
        view: &AuthorityReadView<'_>,
    ) -> Result<FullQueryCapture<PreparedEntryInfo<'permit>>, AuthorityReadError> {
        let cut = view.full_read_cut()?;
        let observed_rows = cut.owner_count();
        if !self.begin_capture(observed_rows)? {
            return Ok(FullQueryCapture::NeedsGrow(observed_rows));
        ...
        let history_count = self
            ...
            .ok_or(AuthorityReadError::Arithmetic)?;
        Ok(FullQueryCapture::Prepared(PreparedEntryInfo {
            permit: self,
            pending_count,
            proposed_count,
            history_count,
    ...
    pub(super) fn capture_fee_estimate<'permit>(
        ...
        snapshot: &Snapshot,
        min_fee_rate: FeeRate,
    ) -> Result<FullQueryCapture<PreparedFeeEstimate<'permit>>, AuthorityReadError> {
        let cut = view.full_read_cut()?;
        let observed_rows = cut.owner_count();
        if !self.begin_capture(observed_rows)? {
            return Ok(FullQueryCapture::NeedsGrow(observed_rows));
        ...
        Ok(FullQueryCapture::Prepared(PreparedFeeEstimate {
            permit: self,
    ...
    fn begin_capture(&mut self, observed_rows: usize) -> Result<bool, AuthorityReadError> {
        if !self.state.rows.is_empty() || observed_rows > self.state.max_rows {
            return Err(AuthorityReadError::Projection);
        }
