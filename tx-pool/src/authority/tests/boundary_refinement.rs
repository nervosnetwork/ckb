...
fn controller_checkpoints(trace: Vec<BoundaryCheckpoint>) -> Vec<BoundaryCheckpoint> {
    trace
        .into_iter()
        .filter(|checkpoint| matches!(checkpoint, BoundaryCheckpoint::Controller { .. }))
        .collect()
}

fn effect_checkpoints(trace: Vec<BoundaryCheckpoint>) -> Vec<BoundaryCheckpoint> {
    trace
        .into_iter()
        .filter(|checkpoint| {
            matches!(
                checkpoint,
                BoundaryCheckpoint::Effect { .. } | BoundaryCheckpoint::Relay { .. }
            )
        })
        .collect()
}
...
async fn uak_controller_remote_handoff_refines_queued_handler_and_response_cuts() {
    ...
    let expected = controller_checkpoints(reference_controller_success_boundary_trace(source));
...
async fn uak_controller_full_and_closed_outcomes_refine_both_relay_sources() {
    for source in [
        ...
        for failure in [BoundaryEnqueueFailure::Full, BoundaryEnqueueFailure::Closed] {
            let expected =
                controller_checkpoints(reference_failed_enqueue_boundary_trace(source, failure));
...
async fn uak_effect_relay_boundaries_refine_publication_and_local_circuit_disposal() {
    for settlement in [
        ...
        let expected = effect_checkpoints(reference_remote_rejection_boundary_trace_with_relay(
            settlement,
        ));
...
async fn uak_topology_lifecycle_refines_running_and_stopped_cuts() {
    ...
    let expected = reference_remote_rejection_boundary_trace_with_relay(
        BoundaryRelaySettlement::ConservativeReset,
    )
    .into_iter()
    .filter(|checkpoint| {
        matches!(
            checkpoint,
            BoundaryCheckpoint::Lifecycle(
                BoundaryLifecycleState::Running | BoundaryLifecycleState::Stopped
            )
        )
    })
    .collect::<Vec<_>>();
