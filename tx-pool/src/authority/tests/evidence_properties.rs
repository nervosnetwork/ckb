...
use super::claim_relations::{
    ClaimAdmissionReceipt, ClaimDependencyCut, ClaimDependencyKey, ClaimDependencyLevel,
    ClaimEvidenceFrontier, ClaimEvidenceIdentity, ClaimEvidenceProof, ClaimEvidenceValidation,
    ClaimEvidenceView, ClaimFinalAdmissionSubject, ClaimKnownDependencies, ClaimMissingDisposition,
    ClaimMissingFact, ClaimPreAcceptedSource, ClaimRawTransaction, ClaimReadyOwner,
    ClaimSubjectValidation, ClaimUnindexedDependencyLevel, missing_resolution_disposition,
...
fn claim_frontier(levels: [AbstractLevel; 2], unindexed: AbstractLevel) -> ClaimEvidenceFrontier {
    let levels = levels.into_iter().enumerate().filter_map(|(index, level)| {
        level.change.map(|change| {
            (
                claim_key(index as u8),
                ClaimDependencyLevel::new(
                    ClaimDependencyCut(change),
                    level.loss.map(ClaimDependencyCut),
                )
                .expect("the finite claim loss does not follow its change"),
            )
        })
    });
    ClaimEvidenceFrontier::new(
        ...
        ClaimUnindexedDependencyLevel::new(
            unindexed.change.map(ClaimDependencyCut),
            unindexed.loss.map(ClaimDependencyCut),
        )
        .expect("the finite claim unindexed level is legal"),
    )
    .expect("the finite claim evidence keys are unique")
}

const fn claim_key(index: u8) -> ClaimDependencyKey {
    match index {
        0 => ClaimDependencyKey::cell(0),
        1 => ClaimDependencyKey::header(1),
...
fn claim_dependencies(mask: u8) -> ClaimKnownDependencies {
    (0..2)
                ...
                for cut in 0..=2 {
                    ...
                    for dependency_mask in 0..4 {
                        ...
                        assert_eq!(
                            ...
                            claim.owner_free_proof_is_current(&claim_dependencies, claim_cut),
                    ...
                    for baseline_mask in 0..4 {
                        ...
                        let claim_baseline = claim_dependencies(baseline_mask);
                        for resolved_mask in 0..4 {
                            let production_resolved = production_dependencies(&keys, resolved_mask);
                            let claim_resolved = claim_dependencies(resolved_mask);
                            assert_eq!(
                            ...
                            for missing_mask in 1..4 {
                                ...
                                let claim_missing = claim_dependencies(missing_mask);
                                assert_eq!(
                                    ...
                                    claim.missing_result_is_current(
                                        &claim_baseline,
                                        &claim_resolved,
                                        &claim_missing,
                                        claim_cut,
                        ...
                        for missing_mask in 1..4 {
                            ...
                            assert_eq!(
                                ...
                                claim.missing_observation_is_current(
                                    &claim_baseline,
                                    &claim_missing,
                                    claim_cut,
                                ),
...
fn abstract_receipt() -> (ClaimEvidenceIdentity, ClaimAdmissionReceipt) {
    ...
    (
        ...
        ClaimAdmissionReceipt {
            proof: ClaimEvidenceProof {
                view: ClaimEvidenceView(0),
                identity,
                dependencies: [ClaimDependencyKey::cell(0)].into_iter().collect(),
                dependency_cut: ClaimDependencyCut(0),
            },
        },
    )
}

fn scenario_frontier(scenario: EvidenceScenario, owner_free: bool) -> ClaimEvidenceFrontier {
    let loss = (scenario == EvidenceScenario::DefinitiveLoss).then_some(ClaimDependencyCut(1));
    let levels = (!owner_free).then(|| {
        (
            ClaimDependencyKey::cell(0),
            ClaimDependencyLevel::new(ClaimDependencyCut(1), loss)
    ...
    ClaimEvidenceFrontier::new(
        levels,
        ClaimUnindexedDependencyLevel::new(
            owner_free.then_some(ClaimDependencyCut(1)),
...
fn production_evidence_observation(result: Result<(), PlanError>) -> ClaimEvidenceValidation {
    match result {
        Ok(()) => ClaimEvidenceValidation::Current,
        Err(PlanError::Stale(StalePlan::ChainRevision)) => ClaimEvidenceValidation::StaleChain,
        Err(PlanError::Stale(StalePlan::Dependency)) => ClaimEvidenceValidation::StaleDependency,
        Err(PlanError::Fault(AuthorityFault::MembershipProjection)) => {
            ClaimEvidenceValidation::StructuralFault
        }
...
fn uak_final_and_direct_acceptance_refine_current_chain_and_dependency_cuts() {
    for scenario in [
        ...
        let (identity, claim_receipt) = abstract_receipt();
        let claim = validate_final_acceptance(
        ...
        assert_eq!(production, claim, "final evidence scenario={scenario:?}");
        ...
        let (_, claim_receipt) = abstract_receipt();
        let claim = validate_direct_acceptance(
            if scenario == EvidenceScenario::StaleChain {
                ClaimEvidenceView(1)
            } else {
                ClaimEvidenceView(0)
        ...
        assert_eq!(production, claim, "direct evidence scenario={scenario:?}");
    }
}
...
fn production_subject_observation(result: Result<(), PlanError>) -> ClaimSubjectValidation {
    match result {
        Ok(()) => ClaimSubjectValidation::Current,
        Err(PlanError::Stale(StalePlan::ChainRevision)) => ClaimSubjectValidation::StaleChain,
        Err(PlanError::Stale(StalePlan::Missing)) => ClaimSubjectValidation::Missing,
        Err(PlanError::Stale(StalePlan::Version)) => ClaimSubjectValidation::StaleVersion,
        Err(PlanError::Stale(StalePlan::Phase)) => ClaimSubjectValidation::StalePhase,
        Err(PlanError::Stale(StalePlan::Dependency)) => ClaimSubjectValidation::StaleDependency,
...
fn uak_final_subject_refines_every_reachable_currentness_outcome() {
    for scenario in [
        ...
        let owners = if scenario == SubjectScenario::Missing {
            ...
            BTreeMap::from([(ClaimRawTransaction(1), owner)])
        };
        let claim = validate_final_subject(
            ...
            ClaimFinalAdmissionSubject {
                view: ClaimEvidenceView(0),
                key: ClaimRawTransaction(1),
                version: 1,
                dependency_cut: ClaimDependencyCut(0),
            },
        );
        assert_eq!(production, claim, "subject scenario={scenario:?}");
    }
}
...
fn claim_key_for_production(
    key: &DependencyKey,
    cell: &OutPoint,
    header: &Byte32,
) -> ClaimDependencyKey {
    match key {
        DependencyKey::Cell(out_point) if out_point == cell => ClaimDependencyKey::cell(0),
        DependencyKey::Header(hash) if hash == header => ClaimDependencyKey::header(1),
...
fn production_missing_observation(
    ...
    match observation {
        MissingResolutionObservationForFoundation::Wait => ClaimMissingDisposition::Wait,
        MissingResolutionObservationForFoundation::RejectUnknownCell(out_point) => {
            ClaimMissingDisposition::RejectUnknownCell(claim_key_for_production(
        ...
        MissingResolutionObservationForFoundation::RejectInvalidHeader(hash) => {
            ClaimMissingDisposition::RejectInvalidHeader(claim_key_for_production(
...
fn source_pair(
    ...
    nonce: u64,
) -> (PreAcceptedSource, ClaimPreAcceptedSource) {
    let admission = match source {
        ClaimPreAcceptedSource::Remote => {
            ValidatedAdmission::remote(tx(nonce), PeerIndex::from(nonce as usize))
        }
        ClaimPreAcceptedSource::Proposal => ValidatedAdmission::proposal(tx(nonce)),
        ClaimPreAcceptedSource::Recovery => {
...
fn uak_missing_dependency_policy_refines_source_and_parent_ownership() {
    ...
    let cases = [
        (
            ...
            BTreeSet::from([ClaimMissingFact::Cell {
                key: ClaimDependencyKey::cell(0),
        ...
        (
            ...
            BTreeSet::from([ClaimMissingFact::Header {
                key: ClaimDependencyKey::header(1),
            }]),
        ),
        (
            vec![
    ...
    for source in [
        ClaimPreAcceptedSource::Remote,
        ClaimPreAcceptedSource::Proposal,
        ClaimPreAcceptedSource::Recovery,
    ] {
        let (production_source, claim_source) = source_pair(source, 6_810 + source as u64);
        for (keys, facts) in &cases {
            ...
            let claim = missing_resolution_disposition(claim_source, facts);
            assert_eq!(
                production, claim,
