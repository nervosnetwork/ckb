...
//! reference model. All production transitions and normalization below are
...
use crate::{
    authority::{
        effect::{
            CommittedAcceptance, CommittedEffect, EffectReceipt, test_support::EffectTraceClass,
        },
    ...
    mathematical_model::{
        ...
        TraceEffect, TraceEffectClaim, TraceEffectClass, TraceEffectObservation,
        TraceLifecycleRoute, TraceObservation, TraceOwnerLocation, TraceOwnerObservation,
        TracePeerId, TraceResourceCounts, TraceRetainedPhase, TraceRetainedSource, TraceScenario,
        TraceTxId, TraceVerifyCapability, TraceVerifyClass, TraceWorkLocation,
        TraceWorkObservation, TraceWorkPermit, TraceWorkStage, replay_reference_trace,
    },
};
...
#[test]
fn uak_authority_lifecycle_traces_refine_model_at_every_stable_cut() {
    for class in TraceVerifyClass::ALL {
        for route in TraceLifecycleRoute::ALL {
            ...
            let expected: Vec<TraceCut> = replay_reference_trace(&scenario)
                .expect("the reference lifecycle trace is legal and invariant-safe");
            let actions = scenario.actions();
            let mut production = ProductionTrace::new(scenario.clone());
            assert_eq!(expected.len(), actions.len());
            for (index, (action, expected_cut)) in actions.iter().copied().zip(expected).enumerate()
            {
                ...
                assert_eq!(
                    actual_disposition,
                    expected_cut.disposition,
                    "disposition divergence at {class:?}/{route:?} cut {index}; prefix={:#?}",
                    &actions[..=index]
                );
                ...
                assert_eq!(
                    actual_observation,
                    expected_cut.observation,
                    "stable-cut divergence at {class:?}/{route:?} cut {index}; prefix={:#?}",
                    &actions[..=index]
                );
            }
        }
