//! Package selection: fees, dependency ordering and block limits.
...
fn uak_template_packer_selects_an_exact_fit_cpfp_package_parent_first() {
    ...
    let parent = accept(&authority, parent_tx.clone(), 1, 10, Status::Proposed);
    ...
    let rival = accept(&authority, rival_tx, 1_000, 30, Status::Proposed);
...
fn uak_template_packer_rescores_descendants_after_shared_parent_selection() {
    ...
    let parent_tx = TransactionBuilder::default()
        ...
        .output_data(Bytes::new().pack())
        .build();
    let parent = accept(&authority, parent_tx.clone(), 1, 1, Status::Proposed);
    let strongest_tx = child_transaction(2_011, &parent_tx);
    let strongest = accept(&authority, strongest_tx, 1_000_000, 1, Status::Proposed);
    let rescored_tx = child_transaction_at(2_012, &parent_tx, 1);
    let rescored = accept(&authority, rescored_tx, 100_000, 1, Status::Proposed);
    let rival_tx = output_transaction(2_013);
    let rival = accept(&authority, rival_tx, 60_000, 1, Status::Proposed);
...
fn uak_template_packer_aggregates_multi_parent_descendant_adjustments() {
    ...
    let child_tx = TransactionBuilder::default()
        ...
        .build();
    let child = accept(&authority, child_tx, 1, 1, Status::Proposed);
...
fn uak_template_packer_requires_proposed_ancestors_and_orders_conditional_edges() {
    ...
    accept(&authority, parent_tx.clone(), 1, 1, Status::Pending);
    let child_tx = child_transaction(2_031, &parent_tx);
    let child = accept(&authority, child_tx, 1_000_000, 1, Status::Proposed);
    ...
    let reader_tx = TransactionBuilder::default()
        ...
        .build();
    let reader = accept(&authority, reader_tx, 1, 1, Status::Proposed);
    let spender_tx = TransactionBuilder::default()
        .version(2_033u32)
        .input(CellInput::new(shared, 0))
        .build();
    let spender = accept(&authority, spender_tx, 1_000_000, 1, Status::Proposed);
