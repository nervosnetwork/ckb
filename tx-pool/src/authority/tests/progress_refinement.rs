...
use crate::{
    ...
    mathematical_model::{
        ...
        EffectPublicationObservation as ModelEffectPublicationObservation,
        EffectReceiptSource as ModelEffectReceiptSource, EffectUsageCut, ProgressVersion,
        SchedulerProgressCut, WakeObservation as ModelWakeObservation,
    },
};
use std::sync::Arc;
...
fn model_cut(input: WakeProjectionInput) -> AuthorityProgressCut {
...
fn assert_wake_refines(before: WakeProjectionInput, after: WakeProjectionInput) {
    ...
    let model = ModelWakeObservation::between(model_cut(before), model_cut(after));
    assert_eq!(
        production,
        ProductionWakeObservation {
            compute: model.compute,
            ready: model.ready,
            dependency_maintenance: model.dependency_maintenance,
            effect_publisher: model.effect_publisher,
            effect_capacity: model.effect_capacity,
...
fn model_effect_observation(
    ...
    match cut.publication_observation() {
        ModelEffectPublicationObservation::Receipt { source, head } => {
            EffectPublicationObservationSnapshot::Receipt {
                source: match source {
                    ModelEffectReceiptSource::Queued => EffectReceiptSourceObservation::Queued,
                    ModelEffectReceiptSource::GenerationReset => {
        ...
        ModelEffectPublicationObservation::Idle => EffectPublicationObservationSnapshot::Idle,
        ModelEffectPublicationObservation::ClosedAndDrained => {
...
fn assert_effect_observation_refines(authority: &TxPoolAuthority) {
    ...
    assert_eq!(
        ...
        model_effect_observation(&stored)
    );
}
