...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn selected_transaction_outside_its_captured_owner_faults_and_releases_template_readers() {
    let driver = fixture();
    let hash = accept(&driver.store, tx(7196), 1, 1, Status::Proposed);
    let owner = driver.store.point(&hash).1.unwrap();
    let mut accepted = owner.accepted().unwrap().clone();
    // Corrupt the accepted projection while retaining the captured owner.
    Arc::make_mut(&mut accepted.transaction).transaction = tx(7197);
    replace(&driver.store, owner, Phase::Accepted(accepted));
    let mut reader = Box::pin(driver.read(deadline()));
    assert!(futures_util::poll!(reader.as_mut()).is_pending());
    assert!(matches!(
...
#[test]
fn uncle_arriving_after_preparation_retries_without_publishing_or_faulting() {
    use ckb_types::core::{BlockBuilder, EpochNumberWithFraction};

    let driver = fixture();
    accept(&driver.store, tx(7198), 1, 1, Status::Pending);
    let mut cache = Cache::default();
    driver.rebuild(&mut cache).unwrap();
    let previous = selected(&driver);
    let prepared = driver.prepare(&mut cache).unwrap();
    ...
    driver
        ...
        .expect("uncle delivery does not change the captured chain or owners");

    assert!(matches!(driver.publish(prepared), Err(Error::Stale)));
    assert!(Arc::ptr_eq(&selected(&driver), &previous));
    assert_eq!(driver.assembler.candidate_uncles.lock().len(), 1);
    assert!(!driver.store.is_faulted());
    driver.rebuild(&mut cache).unwrap();
    assert!(!Arc::ptr_eq(&selected(&driver), &previous));
    assert!(!driver.store.is_faulted());
...
fn prepared_template_cannot_rebind_a_reentered_selected_owner() {
    ...
    let prepared = driver.prepare(&mut cache).unwrap().current;
...
async fn failed_new_source_build_does_not_return_the_previous_tip_template() {
    ...
    driver
        .store
        .apply(chain::clear(&driver.store, Some(unavailable), ClearScope::All).unwrap())
        .unwrap();
    let task = tokio::spawn(Arc::clone(&driver).run());
    assert!(matches!(
...
fn same_view_reuses_only_mandatory_payloads_and_consumes_a_fresh_work_id() {
    ...
    assert!(matches!(
        error,
        BuildError::Assembly(error)
            if matches!(error.downcast_ref::<BlockAssemblerError>(),
...
fn identical_tip_new_view_rebuilds_mandatory_parts_and_enforces_its_byte_limit() {
    ...
    driver
        .store
        .apply(chain::clear(&driver.store, Some(too_small), ClearScope::All).unwrap())
        .unwrap();
    let error = driver
        .prepare(&mut Cache::default())
        .err()
    ...
    assert!(matches!(
        error,
        BuildError::Assembly(error)
            if matches!(error.downcast_ref::<BlockAssemblerError>(), Some(BlockAssemblerError::Overflow))
    ));
}
