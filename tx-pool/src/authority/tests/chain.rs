...
#[tokio::test]
async fn uak_allocation_replacement_retires_the_complete_current_generation() {
    let snapshot = genesis_snapshot();
    let runtime = AuthorityRuntime::new(
        &runtime_config(),
        snapshot.consensus(),
    ...
    runtime
        ...
        .await
        .expect("the allocation terminal owns a fixed carrier replacement without recoverable allocation");
...
#[tokio::test]
async fn uak_scratch_recovery_replacement_preserves_exact_fence_rows_and_facade_identity() {
    let constrained = ResourceLimits::new(
        ResourceVector::new(1, 64 * 1024, 64, 1),
        ResourceVector::new(1, 64 * 1024, 64, 1),
        ResourceVector::new(1, 64 * 1024, 64, 1),
    ...
    .expect("one-at-a-time ingress and larger accepted membership are valid");
    let mut authority = TxPoolAuthority::for_foundation(constrained);
    authority.set_peer_ban_limit_for_foundation(4);
    let fenced_peer = PeerIndex::from(1_310);
    let expired_peer = PeerIndex::from(1_311);
    drop(
        authority
            .plan_peer_revocation_for_foundation(fenced_peer)
            .expect("the fixture installs one persistent routed peer fence")
    ...
    let expired_observation = std::time::Instant::now()
    ...
    drop(
        authority
            .plan_peer_revocation_at_for_foundation(expired_peer, expired_observation)
            .expect("the fixture installs one expired bounded fence")
            .apply(),
    );
    assert!(authority.peer_is_banned_for_reference(fenced_peer));
    assert!(!authority.peer_is_banned_for_reference(expired_peer));
    let parent = output_transaction(1_757);
    let child = child_transaction(1_758, &parent);
    let parent_hash = accept_remote_transaction(
        &mut authority,
    ...
    let fenced_row = live_layout.peer_ingress_row(fenced_peer);
    let expired_row = live_layout.peer_ingress_row(expired_peer);
    ...
    runtime.with_authority_for_foundation(|authority| {
        ...
        assert!(
            entries.same_layout_for_test(&live_layout),
            "scratch recovery swaps the payload inside the same physical layout, so facades cannot go stale"
        );
        assert_eq!(
            entries.peer_ingress_row(fenced_peer),
            fenced_row,
            "the active fence row keeps its exact lease and revision across scratch replacement"
        );
        assert_eq!(
            entries.peer_ingress_row(expired_peer),
            expired_row,
            "the expired fence row keeps its exact lease and revision across scratch replacement"
        );
        assert_eq!(
            entries.generation_payload_swaps_for_test(),
            swaps_before + crate::authority::shard::AUTHORITY_SHARD_COUNT,
            "scratch replacement performs exactly 64 payload swaps"
        );
        assert!(authority.peer_is_banned_for_reference(fenced_peer));
        assert!(!authority.peer_is_banned_for_reference(expired_peer));
        assert_eq!(authority.owner_count(), 1);
        assert!(
            authority.entry(&parent_hash).is_some(),
            "the recovered owner enters the new generation beside the persistent fences"
        );
        assert_resource_reference(authority);
    });
}
...
fn uak_unrepresentable_recovery_set_converges_to_a_fresh_parent_first_prefix() {
    ...
    let committed = authority
        .plan_chain_generation_replacement_preserving_sources(next, fallback)
