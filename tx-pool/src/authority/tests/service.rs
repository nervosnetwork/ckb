...
async fn service_assembly_with_config_and_recent_reject(
    ...
    service_assembly_with_command(config, recent_reject, ChunkCommand::Resume).await
}

async fn service_assembly_with_command(
    config: TxPoolConfig,
    recent_reject: Option<Arc<RecentReject>>,
    ...
    let (verification_control, _command_tx) =
        AuthorityVerificationControl::channel(initial_command);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn uak_service_shared_contention_is_negative_ack_without_ban_or_generation_loss() {
    const PHASE_TIMEOUT: Duration = Duration::from_secs(5);
    let (assembly, snapshot, _relay) =
        service_assembly_with_command(runtime_config(), None, ChunkCommand::Suspend).await;
    let runtime = assembly.service.runtime_for_foundation();
    let peer = PeerIndex::from(42);
    let transaction = TransactionBuilder::default()
        .input(CellInput::new(OutPoint::new(Byte32::new([42; 32]), 0), 0))
        .output(
    ...
    crate::util::non_contextual_verify(snapshot.consensus(), &transaction)
        .expect("the contention fixture reaches validated retained ingress");
    ...
    let contended_service = assembly.service.clone();
    let contended_tx = transaction.clone();
    let executor = tokio::runtime::Handle::current();
    let contended = std::thread::spawn(move || {
        executor.block_on(contended_service.submit_remote(
            bounded_transaction(contended_tx),
            0,
            peer,
        ))
    });
    if let Err(error) = entered.recv_timeout(PHASE_TIMEOUT) {
        let _ = release.send(());
        panic!("the first service request must classify an absent owner: {error}");
    }
    runtime.with_authority_read_for_foundation(|authority| {
        authority.entries_for_reference().set_shared_ingress_probe(
            SharedIngressProbePhase::AfterRetainedIngressHeadClassification,
    ...
    let winner = assembly
        .service
        .submit_remote(bounded_transaction(transaction), 0, peer)
        .await;
    release.send(()).expect("release the stale service request");
    winner.expect("the competing shared owner insertion commits");
    let contended = contended.join().expect("the service request thread joins");
    assert!(
        matches!(contended, Err(AuthorityServiceError::ResourceUnavailable)),
        "shared contention must remain an operational negative terminal: {contended:?}"
    );
    runtime.with_authority_read_for_foundation(|authority| {
        assert!(authority.entry(&hash).is_some());
        assert!(!authority.peer_is_banned_for_reference(peer));
        let inconsistencies = authority.primary_projection_inconsistencies();
        assert!(
            inconsistencies.is_empty(),
            "the surviving owner keeps every production projection: {inconsistencies:?}"
        );
    });
...
async fn uak_service_boundary_closes_administration_and_ordered_chain_commands() {
    ...
    assert!(
        !assembly
            ...
            .expect("an absent removal cannot lose a present-owner cut")
    );
    assembly
        .service
        .clear_pipeline()
