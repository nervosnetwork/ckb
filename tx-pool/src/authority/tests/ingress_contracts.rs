...
fn refused_plan_retains_its_original_reads_and_discards_speculative_changes_and_effects() {
    for changed in [false, true] {
        ...
        plan.edit(
            Some(Arc::clone(&old)),
            None,
            Some(Effect::removed(&old, Reject::Expiry(1), None).unwrap()),
        )
        .unwrap();
        let (view, snapshot) = store.snapshot();
        plan.replace_generation(snapshot);
        plan.ban_peer(
            &candidate.hash(),
            malformed(),
            91.into(),
...
fn active_peer_ban_survives_both_full_and_pipeline_clear() {
    for scope in [ClearScope::All, ClearScope::Unaccepted] {
        let store = store();
        let mut ban = Plan::new(store.snapshot().0, Class::Remote, Default::default());
        ban.ban_peer(
            &tx(0).hash(),
        ...
        store.apply(ban).unwrap();
        store
            .apply(super::super::chain::clear(&store, None, scope).unwrap())
            .unwrap();
