...
use crate::authority::model::RemoteOrigin;
use crate::{
...
async fn zero_workers_leave_remote_work_queued_while_direct_submission_completes() {
    ...
    .unwrap();
    let remote = fund(&pool, 9040);
    let local = fund(&pool, 9041);
    let cycles = within(pool.test_accept(bounded(remote.clone())))
        .await
        .unwrap()
        .unwrap()
        .cycles;
    ...
    within(pool.submit_remote(bounded(remote.clone()), cycles, 95.into()))
        .await
        .unwrap();
    within(pool.submit_local(bounded(local.clone())))
        .await
        .unwrap()
        .unwrap();
    assert!(
    ...
    // Joining the real background tasks closes the observation interval without
    // a sleep: no verifier can advance the retained network owner afterward.
    shutdown(&pool, tasks, publisher).await;
    assert!(matches!(
        pool.store.point(&remote.hash()).1.unwrap().phase,
        Phase::Resolve
    ));
    assert_eq!(pool.store.capture_summary().queued, 1);
}
...
async fn dry_run_under_active_pressure_returns_without_owner_or_publication_changes() {
    ...
    let all_cpu = Arc::clone(&pool.cpu)
        ...
        .unwrap();
    let before = pool.store.capture_all().owners.len();
    assert!(matches!(
        within(pool.test_accept(bounded(tx))).await.unwrap(),
        Err(Reject::Full(_))
    ));
    assert_eq!(pool.store.capture_all().owners.len(), before);
    assert!(!pool.is_faulted());
...
async fn stop_joins_a_suspended_verification_queue_and_preserves_accepted_state() {
    ...
    shutdown(&pool, tasks, publisher).await;
    assert_eq!(pool.store.capture_accepted().owners.len(), 1);
...
async fn public_callback_queries_dry_runs_and_rejects_reentrant_mutation_then_service_joins() {
    ...
    // Only direct submissions are under test. Keep nonwaiting callback previews
    // independent of idle workers briefly acquiring computation permits.
    configuration.max_tx_verify_workers = 0;
    let handle = Handle::new(tokio::runtime::Handle::current(), None);
...
async fn internal_insertion_cannot_replace_or_evict_an_existing_owner() {
    for replacement in [false, true] {
        ...
        assert_eq!(pool.store.capture_all().owners.len(), 1);
        pool.close_outbox();
        assert!(pool.store.outbox.drained());
        assert!(!pool.is_faulted());
    }
...
async fn remote_waiting_transaction_accepts_after_its_parent_completes_normal_verification() {
    ...
    let cycles = pool
        ...
        .unwrap()
        .cycles;
    // Preview does not wait for computation capacity. Check missing-dependency
    // validation before background workers can compete for that capacity.
    let preview = within(pool.test_accept(bounded(child.clone())))
        .await
        .unwrap();
    assert!(
        matches!(
        ...
        "missing parent must fail preview resolution: {preview:?}"
    );
    assert!(pool.store.point(&child.hash()).1.is_none());
    let (_chain, chain) = mpsc::channel(2);
    let (tasks, publisher) =
        pool.start_background(&handle, endpoints(sink, Callbacks::new()), chain);
    let submitted = within(pool.submit_local(bounded(child.clone())))
        .await
        .unwrap();
    assert!(
        matches!(
        ...
        "missing parent must fail submission resolution: {submitted:?}"
    );
    assert!(pool.store.point(&child.hash()).1.is_none());
    assert!(matches!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn requeue_retries_concurrent_chain_changes_even_during_stop() {
    for stop in [false, true] {
        ...
        *pool.store.commit_observer.lock() = None;
        ...
        assert_eq!(successor.arrival, original.arrival);
        assert_eq!(pool.store.capture_summary().queued, 1);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn requeue_preserves_a_concurrent_successor_or_removal() {
    for remove in [false, true] {
        ...
        let applying = Arc::downgrade(&pool);
        let next = successor.clone();
        *pool.store.commit_observer.lock() = Some(Arc::new(move |_, locked| {
            if !locked {
                let pool = applying.upgrade().unwrap();
                *pool.store.commit_observer.lock() = None;
                let mut plan =
                    Plan::new(pool.store.snapshot().0, Class::Critical, Default::default());
                plan.edit(Some(Arc::clone(&original)), next.clone(), None)
                    .unwrap();
                pool.store.apply(plan).unwrap();
            }
        }));
        ...
        if let Some(successor) = successor {
            ...
            assert!(pool.store.point(&selected.entry.hash()).1.is_none());
            assert_eq!(pool.store.capture_summary().queued, 0);
        }
        selected.mark_handled();
        drop(selected);
        assert!(pool.store.budget.active_is_empty_for_test());
...
async fn peer_pressure_refuses_rbf_without_losing_victim_and_same_peer_retry_succeeds() {
    ...
    assert!(!pool.store.peer_banned(peer));
    drop(victim_owner);
    within(pool.clear(None, ClearScope::All)).await.unwrap();
    shutdown(&pool, tasks, publisher).await;
...
async fn direct_non_contextual_rejection_preserves_empty_membership_for_send_and_dry_run() {
    ...
    assert!(pool.store.capture_all().owners.is_empty());
    drop(occupied);
    for dry_run in [true, false] {
        let result = within(async {
            if dry_run {
...
async fn optional_history_pressure_preserves_local_and_owned_job_admission() {
    for (local, stop) in [(false, false), (true, false), (false, true), (true, true)] {
        ...
        let verified = verification_fixture(&pool.store, &candidate, 10_000, 1, Status::Pending);
        ...
        *pool.store.commit_observer.lock() = Some(Arc::new(move |plan, locked| {
            if !plan.edits().get(&target).is_some_and(|edit| {
                edit.after
                    .as_ref()
            ...
            if !locked {
                ...
                if !local && !stop && (1..=3).contains(&attempt) {
                    ...
                    // plans. Verification still holds; admission must finish
                    // after this bounded interference ends.
                    let owner = pool.store.point(&victim).1.unwrap();
                    let mut value = owner.accepted().unwrap().clone();
                    value.timestamp += 1;
                    replace(&pool.store, owner, Phase::Accepted(value));
                }
...
async fn public_queries_expose_acceptance_and_count_remaining_verification_work() {
    ...
    let verified = verification_fixture(&pool.store, &candidate, 1, 7, Status::Pending);
    let queued = replace(
        &pool.store,
        candidate,
    ...
    pool.accept_job(&job, &verified).await.unwrap();
    job.mark_handled();
    drop(job);
    assert_eq!(
        pool.transaction_status(&queued.hash()).unwrap(),
...
async fn local_removal_completes_the_accepted_closure_beyond_an_expiry_page() {
    ...
    let remaining = pool.store.capture_accepted().owners;
...
async fn admitted_ingress_batch_returns_a_poll_boundary_and_preserves_the_stopped_prefix() {
    for proposal in [false, true] {
        ...
        let handler = tokio::spawn(async move {
            ...
            let installed = running.store.capture_all().owners.len();
        ...
        assert_eq!(pool.store.capture_all().owners.len(), installed);
        assert!(pool.verification.resume().is_err());
        pool.close_outbox();
        assert!(pool.persistence_eligible());
        assert!(!pool.is_faulted());
