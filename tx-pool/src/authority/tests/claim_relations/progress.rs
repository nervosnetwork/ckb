//! Finite claim relations for post-commit wake and effect publication.
//!
...
pub(crate) struct EffectLogCut {
    ...
    pub(crate) blocking_staged_head: Option<EffectHead>,
...
impl EffectLogCut {
    ...
    pub(crate) fn publication_observation(self) -> EffectPublicationObservation {
        ...
        if self.blocking_staged_head.is_some_and(|staged| {
            receipt.is_none_or(|(_, candidate)| staged.sequence < candidate.sequence)
        }) {
            return EffectPublicationObservation::Idle;
        }
        if let Some((source, head)) = receipt {
            return EffectPublicationObservation::Receipt { source, head };
        }
        if self.closed
            && self.blocking_staged_head.is_none()
            && self.pending_recent_rejects == 0
            && self.usage.is_empty()
        {
