...
pub(crate) fn scheduler_wave_observation(
    ...
    let capability = |role| match role {
    ...
    let mut selected = vec![None; slots.len()];
    for (index, worker) in slots.iter().enumerate() {
        if matches!(worker.role, SchedulerRefinementWorkerRole::OrderedResolve) {
            continue;
        }
        selected[index] = select(
            &mut remaining,
            Lane::Verify,
            capability(worker.role),
            order,
            &mut cursors,
        );
    }
    for (index, worker) in slots.iter().enumerate() {
        if selected[index].is_some()
            || matches!(worker.role, SchedulerRefinementWorkerRole::OrderedResolve)
        {
            continue;
        }
        selected[index] = select(
    ...
    for (worker, selected) in slots.into_iter().zip(selected) {
...
const fn worker_rank(role: SchedulerRefinementWorkerRole) -> u8 {
    match role {
        SchedulerRefinementWorkerRole::VerifySmall => 0,
        SchedulerRefinementWorkerRole::VerifyAny => 1,
        SchedulerRefinementWorkerRole::OrderedResolve => 2,
    }
}
