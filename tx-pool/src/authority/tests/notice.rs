...
#[test]
fn removal_notices_follow_owner_phase_and_origin() {
    let store = store();
    let hash = accept(&store, output_tx(9500), 1, 1, Status::Pending);
    let accepted = store.point(&hash).1.unwrap();
    ...
    for (source, remote) in [
        ...
        for was_accepted in [false, true] {
            let owner = Entry {
                transaction: Arc::clone(&accepted.transaction),
                arrival: accepted.arrival,
                source,
                phase: if was_accepted {
                    accepted.phase.clone()
                } else {
                    Phase::Resolve
                },
            };
            ...
            if let Some(relay) = &effect.relay {
                assert!(
                    matches!(relay, TxVerificationResult::Reject { tx_hash } if *tx_hash == hash)
                );
            }
            assert_eq!(effect.callback.is_some(), was_accepted, "{source:?}");
            if let Some(callback) = &effect.callback {
                assert!(
                    matches!(callback, CallbackEvent::Reject(entry, Reject::Expiry(0))
...
async fn older_acknowledgement_cannot_erase_later_pending_rejection() {
    ...
    assert_eq!(
        ...
        serde_json::json!({"type": "Malformed", "description": "Malformed second transaction"}),
    );
    second.activate(&outbox);
    outbox.close();
    publisher.await.unwrap();
...
fn rejection_diagnostics_keep_policy_public_shape_and_bounded_owned_strings() {
    ...
    for original in cases {
        ...
        let public = PoolTransactionReject::try_from(original.clone()).unwrap();
        let bounded = bound_reject_diagnostic(original, MAX_DYNAMIC_REJECT_TEXT_BYTES);
        assert_eq!(bounded.to_string(), expected);
        assert_eq!(PoolTransactionReject::try_from(bounded).unwrap(), public);
    }
    let mut text = String::with_capacity(MAX_TX_POOL_REJECT_DESCRIPTION_BYTES * 4);
    text.push_str("short transient diagnostic");
    let Reject::Full(text) =
        bound_reject_diagnostic(Reject::Full(text), MAX_DYNAMIC_REJECT_TEXT_BYTES)
    else {
        panic!("same variant")
    };
    assert_eq!(text, "short transient diagnostic");
    assert!(text.capacity() <= MAX_TX_POOL_REJECT_DESCRIPTION_BYTES);
    let rejection = bound_reject_diagnostic(
        Reject::RBFRejected("界".repeat(MAX_TX_POOL_REJECT_DESCRIPTION_BYTES)),
...
#[test]
fn escaped_rejection_records_keep_the_original_public_type_within_the_byte_limit() {
    let message = "\u{0}".repeat(MAX_TX_POOL_REJECT_DESCRIPTION_BYTES);
    let cases = [
        Reject::Full(message.clone()),
        Reject::Malformed(message.clone(), message.clone()),
        Reject::RBFRejected(message.clone()),
        Reject::Invalidated(message.clone()),
        Reject::Verification(ckb_error::ErrorKind::Transaction.other(message)),
    ];
    for reject in cases {
        let public = PoolTransactionReject::try_from(reject.clone()).unwrap();
        let original = serde_json::to_value(public).unwrap();
        let effect = Effect::rejected(&tx(9).hash(), reject, None, false).unwrap();
        let (_, encoded) = effect.recent().unwrap();
        assert!(encoded.len() <= MAX_TX_POOL_REJECT_DESCRIPTION_BYTES);
        let recorded: serde_json::Value = serde_json::from_str(encoded).unwrap();
        assert_eq!(recorded["type"], original["type"]);
    }
}

#[test]
fn interrupted_verification_releases_relay_without_persistent_rejection() {
    for reject in [
        Reject::ExcessiveVerifyTime,
        Reject::Verification(ckb_script::ScriptError::Interrupts.unknown_source().into()),
    ] {
        let effect = Effect::rejected(&tx(10).hash(), reject, None, true).unwrap();
        assert!(effect.recent().is_none());
        assert!(effect.callback.is_none());
        assert!(effect.ban.is_none());
        assert!(matches!(
            effect.relay,
            Some(TxVerificationResult::Reject { .. })
        ));
        let detached = effect.rejection.unwrap().reason;
        assert!(detached.is_verification_interrupted());
        assert!(PoolTransactionReject::try_from(detached).is_err());
    }
}
...
async fn rbf_candidate_diagnostic_preserves_victim_callback_and_both_recent_outcomes() {
    ...
    for hash in [&candidate, &victim] {
        let encoded = outbox.pending_reject(hash).unwrap();
        let public = PoolTransactionReject::try_from(reason.clone()).unwrap();
        assert_eq!(encoded, serde_json::to_string(&public).unwrap());
    }
    batch.activate(&outbox);
    outbox.close();
    Arc::clone(&outbox).run(endpoints).await.unwrap();
