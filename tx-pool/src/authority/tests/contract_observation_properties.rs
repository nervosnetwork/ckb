...
#[test]
fn uak_shared_remote_expiry_refines_the_no_interleave_single_owner_fold() {
    let fixture = || {
        let mut authority = TxPoolAuthority::for_foundation(limits());
        let hashes = vec![
    ...
    let (aggregate, aggregate_hashes) = fixture();
    let batch_sequence = aggregate.clocks().next_sequence;
    let aggregate_committed = aggregate
        .plan_remote_expiry(
            RemoteDeadline(10),
            NonZeroUsize::new(3).expect("three is non-zero"),
        )
        .expect("the aggregate expiry plans")
        .expect("three owners are due")
        .apply_for_foundation(&aggregate);
    assert_eq!(aggregate_committed.retired_len(), 3);

    let (reference, reference_hashes) = fixture();
    assert_eq!(reference_hashes, aggregate_hashes);
    for _ in 0..3 {
        let committed = reference
            ...
            .expect("the next single-owner reference prefix plans")
            .expect("one reference owner remains due")
            .apply_for_foundation(&reference);
        assert_eq!(committed.retired_len(), 1);
    }
    ...
    let canonical_next_sequence = ApplySequence(
        batch_sequence
            .0
            .checked_add(3)
            .expect("three reference Applies fit the sequence domain"),
    );
    assert_eq!(
    ...
    assert!(reference.primary_projection_consistent());
}
