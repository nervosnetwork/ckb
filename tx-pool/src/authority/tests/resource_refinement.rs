...
use crate::{
    ...
    mathematical_model::{
        ...
        ContinuousResourceLimits, ContinuousResourceUsage, ContinuousResourceVector,
        ModelComputeGrant,
    },
};
...
fn model_configuration_observation(
...
fn assert_ledger_matches_model(
    production: &ResourceLedger,
    model: &ContinuousResourceLedger,
    keys: impl IntoIterator<Item = u8>,
) {
    let (charges, usage) = production_usage(production, keys);
    assert_eq!(&charges, model.charges());
    assert_eq!(
        usage,
        model.usage().expect("a planned model ledger is valid")
    );
}
...
fn uak_resource_vectors_and_configuration_refine_the_finite_algebra_exhaustively() {
    ...
    for mask in 0u16..(1u16 << 13) {
        ...
        assert_eq!(
            ...
            model_configuration_observation(validate_model_limits(candidate)),
            "configuration premise mask {mask:#x}"
        );
    }

    let model_overflow = ContinuousResourceLimits::validate(
        ContinuousResourceVector {
            entries: 1,
            bytes: u16::MAX,
            edges: 1,
    ...
    assert_eq!(
        model_configuration_observation(model_overflow),
                ...
                for compute_edges in 0..=2 {
                    for residency_peer in [None, Some(1)] {
                        for compute_peer in [None, Some(1), Some(2)] {
                            let model = ContinuousChargeRecord::PreAccepted {
                            ...
                            let production = production_ledger(wide, &BTreeMap::from([(0, model)]));
                            assert_eq!(
                                production.is_ok(),
                                model.validate().is_ok(),
                                "charge validation mismatch for {model:?}"
                            );
                        }
                    }
                }
        ...
        for active_work in 0..=2 {
            for compute_bytes in 0..=2 {
                for compute_edges in 0..=2 {
                    ...
                    assert_eq!(
                        production_ledger(wide, &BTreeMap::from([(0, model)])).is_ok(),
                        model.validate().is_ok(),
                        "history validation mismatch for {model:?}"
                    );
                }
            }
        }
    ...
    for limits in [base_limits(2), base_limits(1)] {
        for initial_left in &options {
            for initial_right in &options {
                ...
                let model = ContinuousResourceLedger::new(limits, initial_map.clone());
                let production = production_ledger(limits, &initial_map);
                assert_eq!(production.is_ok(), model.is_ok());
                let (Ok(model), Ok(_)) = (model, production) else {
                    continue;
                };
                for after_left in &options {
                    for after_right in &options {
                        ...
                        let expected = model.plan_changes(&changes);
                        ...
                        match (plan, expected) {
                            (Ok(plan), Ok(expected)) => {
                                ...
                                assert_ledger_matches_model(&actual, &expected, [0, 1]);
...
#[test]
fn uak_compute_release_refines_exact_reservation_and_peer_attribution_exhaustively() {
    let limits = base_limits(2);
    let records = charge_domain();
    for expected in &records {
        let initial = BTreeMap::from([(0, *expected)]);
        let model = ContinuousResourceLedger::new(limits, initial.clone());
        let production = production_ledger(limits, &initial);
        let (Ok(model), Ok(_)) = (model, production) else {
            continue;
        };
        for after in &records {
            let expected_result = model.plan_compute_release(0, *expected, *after);
            let mut actual = production_ledger(limits, &initial)
            ...
            match (plan, expected_result) {
                (Ok(plan), Ok(expected_result)) => {
                    actual.apply(plan);
                    assert_ledger_matches_model(&actual, &expected_result, [0]);
                }
                (Err(_), Err(_)) => assert_eq!(actual.snapshot(), before),
                _ => unreachable!("success parity was checked above"),
            }
