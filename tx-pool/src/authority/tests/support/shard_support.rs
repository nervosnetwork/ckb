...
/// Test-only deterministic logical-domain support.
///
/// This is not the production randomized physical router and cannot prove a
/// production shard cut. It only checks that an `EntryDelta` names each logical
/// domain it can touch without accepting a caller-provided route vector.
