...
use crate::authority::plan::StalePlan;
use ckb_types::core::TransactionView;

impl AuthorityRuntime {
    pub(in crate::authority) fn lifecycle_writer_active_for_foundation(&self) -> bool {
        self.lifecycle_fence.state.lock().lifecycle_writer_active
    }

    pub(in crate::authority) fn direct_candidate_for_foundation(
        &self,
        tx: std::sync::Arc<TransactionView>,
        verified: super::super::state::VerifiedFacts,
    ) -> AuthorityDirectVerifiedCandidate {
        AuthorityDirectVerifiedCandidate {
            candidate: super::super::resolver::DirectVerifiedCandidate::local_for_foundation(
                tx, verified,
            ),
            execution: self
                .try_compute_execution_for_foundation()
                .expect("the foundation runtime has one Direct execution slot"),
        }
    }
...
impl AuthorityComputeJob {
    ...
    pub(in crate::authority) fn missing_for_foundation(
        ...
        let settlement = match self.inner {
            AuthorityComputeKind::Resolution(job) => job.missing_for_foundation(keys),
            AuthorityComputeKind::Verification(_) => {
                panic!("only Resolve work can produce a missing-dependency settlement")
            }
        };
        AuthorityComputeSettlement {
            settlement,
...
impl AuthorityComputeSettlement {
    pub(in crate::authority) fn into_exchange_completion_for_foundation(
        self,
        ...
        ComputeExchangeCompletion::from_finished(slot, completion.finish_execution())
    }
}
...
impl AuthorityRuntime {
    ...
    pub(in crate::authority) fn new_with_resource_limits_for_foundation(
        config: &TxPoolConfig,
        consensus: &Consensus,
        snapshot: Arc<Snapshot>,
    ...
    pub(in crate::authority) fn commit_retained_ingress_rejection(
        ...
        if consumed != 1 || !remaining.is_empty() {
    ...
    > {
        ...
        assert_eq!(self.publish_committed(checkout_retirement), None);
        if let Some(settlement_retirement) = settlement_retirement {
            assert_eq!(self.publish_committed(settlement_retirement), None);
        }
    ...
    pub(in crate::authority) fn with_authority_read_for_foundation<T>(
        &self,
        inspect: impl FnOnce(&TxPoolAuthority) -> T,
    ) -> T {
        inspect(&self.store.read().authority)
    }

    pub(in crate::authority) fn generation_write_available_for_foundation(&self) -> bool {
        self.store.inner.try_write().is_some()
    }
    ...
    pub(in crate::authority) fn queue_effects_for_foundation(
        ...
        assert_eq!(self.publish_committed(retirement), None);
        Ok(())
    }
        ...
        let committed = {
            ...
            let prepared = store
                ...
                .map_err(|error| match error {
                    ...
                    ConcurrentIndependentError::Fault(fault) => PlanError::Fault(fault),
                })?;
            prepared.map(|committed| committed.into_parts())
        };
        let Some((committed, post_commit_fault)) = committed else {
            return Ok(false);
        };
        assert_eq!(self.publish_committed(committed), None);
