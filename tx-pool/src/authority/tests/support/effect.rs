...
pub(in crate::authority) struct EffectSnapshot {
    ...
    staged: Vec<StagedEffectSnapshot>,
...
pub(in crate::authority) struct EffectObservation {
    ...
    pub(in crate::authority) blocking_staged_head: Option<ApplySequence>,
...
impl EffectLog {
    pub(in crate::authority) fn for_foundation() -> Self {
        ...
        Self {
            ...
            staged: Vec::with_capacity(MAX_STAGED_EFFECTS),
    ...
    pub(in crate::authority) fn trace_batches(&self) -> Vec<EffectTraceBatch> {
        ...
        batches.extend(
            self.staged
                .iter()
                .filter(|staged| staged.record.record.batch.stage_state() == EFFECT_STAGE_COMMITTED)
                .map(|staged| EffectTraceBatch {
                    sequence: staged.record.record.sequence,
                    class: Some(staged.record.class.into()),
                    processed_steps: staged.record.record.processed.0,
                    effects: staged.record.record.batch.effects().to_vec(),
                }),
        );
    ...
    pub(in crate::authority) fn snapshot(&self) -> EffectSnapshot {
        ...
        EffectSnapshot {
            ...
            staged: self
                .staged
                .iter()
                .map(|staged| StagedEffectSnapshot {
                    record: staged.record.clone(),
                    state: staged.record.record.batch.stage_state(),
                })
                .collect(),
    ...
    pub(in crate::authority) fn semantically_consistent(
        ...
        let mut previous = self.queued.back().map(|queued| queued.record.sequence);
        let staged_ordered = self.staged.iter().all(|staged| {
            if previous.is_some_and(|previous| previous >= staged.record.record.sequence) {
                false
            } else {
                previous = Some(staged.record.record.sequence);
                true
            }
        });
        if !staged_ordered {
        ...
        for staged in &self.staged {
            let state = staged.record.record.batch.stage_state();
            if state == EFFECT_STAGE_UNSTAGED
                || state > EFFECT_STAGE_CANCELLED
                || staged.record.record.processed != EffectProgress::default()
            {
                return false;
            }
            if state == EFFECT_STAGE_CANCELLED {
                continue;
            }
            let Some(next) = rebuilt.checked_charge(
                staged.record.class,
                staged.record.record.batch.charge_bytes(),
            ) else {
                return false;
            };
            rebuilt = next;
        ...
        let staged_sequences_before_clock = self
            .staged
            .iter()
            .all(|staged| staged.record.record.sequence < next_sequence);
