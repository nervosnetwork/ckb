...
impl FairLane {
    fn next_after_excluding_for_reference(
        ...
        for _ in 0..self.owner_count(lane, capability) {
            let owner = self.next_owner(lane, capability, cursor, staged)?;
            if let Some(key) = self.by_owner.get(&owner).and_then(|entries| {
                entries.head_excluding(lane, capability, excluded_versions, staged)
            }) {
    ...
    fn next_excluding_for_reference(
        ...
        if cursor.is_none()
            ...
            && let Some(key) = self.by_owner.get(&WorkOwner::Trusted).and_then(|entries| {
                entries.head_excluding(lane, capability, excluded_versions, staged)
            })
        {
            return Some((WorkOwner::Trusted, key));
        }
        self.next_after_excluding_for_reference(lane, capability, cursor, excluded_versions, staged)
    }
...
impl FairFrontier {
    ...
    fn plan_worker_wave(
        ...
        ordered.sort_unstable_by_key(|slot| slot.work_selection_key());
        ...
        selected
            ...
            .map_err(|_| SchedulerError::Allocation)?;
        selected.resize_with(ordered.len(), || None);
        for (index, slot) in ordered.iter().copied().enumerate() {
            if slot.fallback_permit().is_none() {
                continue;
            }
            let primary = slot.primary_permit();
            let Some(ticket) = self.next_queued_in_wave_for_reference(&cursor, primary) else {
                continue;
            };
            cursor.select(&ticket)?;
            selected[index] = Some((primary, ticket));
        }
        for (index, slot) in ordered.iter().copied().enumerate() {
            ...
            let Some(ticket) = self.next_queued_in_wave_for_reference(&cursor, fallback) else {
                continue;
            };
            cursor.select(&ticket)?;
            selected[index] = Some((fallback, ticket));
        }
        for (index, slot) in ordered.iter().copied().enumerate() {
        ...
        idle.try_reserve(ordered.len())
            .map_err(|_| SchedulerError::Allocation)?;
        for (slot, selected) in ordered.into_iter().zip(selected) {
    ...
    pub(in crate::authority) fn semantically_matches(
        ...
        self.semantically_matches_snapshot(&entries.snapshot_for_test())
    }
