...
impl AuthorityService {
    pub(crate) async fn apply_chain_update(
        ...
        let committed = self.commit_chain_update(arguments).await?;
        match self.publish_chain_observers(committed) {
            Some(fault) => Err(super::AuthorityChainUpdateError::integrity(
                super::AuthorityIntegrityFault::from(fault),
            )),
            None => Ok(()),
        }
    }
