...
fn exhausted_wake_counter_faults_and_notifies_every_progress_waiter() {
    ...
    for (name, future) in &mut waiters {
        assert!(future.as_mut().poll(&mut context).is_pending(), "{name}");
    }
    store.start_wake(&key);
    assert!(store.is_faulted());
    for (name, future) in &mut waiters {
        assert!(future.as_mut().poll(&mut context).is_ready(), "{name}");
    }
}
...
fn owner_edit_groups_cover_mixed_guards_and_empty_clear_shards() {
    ...
    let Captured { view, reads, .. } = store.capture_all();
...
fn owner_preflight_preserves_shard_order_between_collision_and_counter_errors() {
    for collision_first in [false, true] {
        ...
        assert!(
            store
                .read_selected(before.view, &before.reads, || ())
                .is_ok()
        );
        assert!(!store.is_faulted());
    }
...
fn remaining_counter_exhaustion_rejects_before_any_owner_or_notice_change() {
    ...
    for counter in [
        ...
        let before = store.capture_all();
        let charge = store.budget.accepted_usage();
        let mut plan = Plan::new(before.view, Class::Trusted, Default::default());
        plan.edit(
        ...
        if counter == "view counter" {
            plan.chain(Arc::clone(&before.snapshot), []);
        }
    ...
    *store.commit_observer.lock() = None;
    for (index, hash) in hashes.into_iter().enumerate() {
        ...
        if removal {
            ...
            assert!(matches!(current.source, Source::Remote { origin, .. }
                if origin.peer == (95 + if same_peer { 0 } else { index }).into()));
        } else {
            assert!(
                matches!(current.unwrap().source, Source::Proposal { remote: Some(origin) } if origin.peer == 95.into())
            );
        }
    }
    pool.stop();
...
fn phase_and_source_updates_keep_current_proposal_and_exact_expiry() {
    for accept_final in [false, true] {
        ...
        let phase = Phase::Verify(Arc::clone(
            verification_fixture(&store, &owner, 1, 1, Status::Pending).resolved(),
        ));
        ...
        let clear = crate::authority::chain::clear(&store, None, ClearScope::All).unwrap();
        store.apply(clear).unwrap();
    ...
    for fee in [1, 1_000] {
        ...
        *store.commit_observer.lock() = Some(Arc::new(move |plan, locked| {
        ...
        let incumbent = std::thread::scope(|scope| {
            ...
            assert!(store.capture_accepted().owners.is_empty());
        ...
        *store.commit_observer.lock() = None;
        ...
        let accepted = store.capture_accepted().owners;
...
fn settled_capacity_refusal_and_dry_run_preserve_the_entire_owner_cut() {
    ...
    assert!(reject.is_none());
    let empty = store.capture_all().reads;
    ...
    for capture in [Store::capture_accepted, Store::capture_all] {
        let Captured {
            view,
            owners,
            reads,
...
fn roleless_owner_transition_combines_duplicate_roles_and_preserves_waiters() {
    ...
    let verified = verification_fixture(&store, &pending, 1, 1, Status::Pending);
