...
use super::super::resources::{
    AcceptedCost, AcceptedResources, ChargeRecord, ResourceLedger, ResourceLimits,
    ResourceSnapshot, ResourceVector,
};
use super::super::state::{
    AcceptedEntry, AcceptedStatus, ActiveWork, AdmissionClass, ApplySequence, CandidateMetrics,
    ChainEpoch, ComputeLeaseId, ComputedOutcome, DependencyEpoch, DependencyKey, EntryVersion,
    ExpandedFootprint, FootprintError, IngressAttribution, InputEvidenceError,
...
fn limits() -> ResourceLimits {
    ResourceLimits {
        preaccepted: ResourceVector::new(8, 64 * 1024, 64, 8),
        remote: ResourceVector::new(4, 32 * 1024, 32, 4),
        per_peer: ResourceVector::new(2, 16 * 1024, 16, 2),
        accepted: AcceptedResources::new(8, 64 * 1024, 64 * 1024, 64),
    }
}
...
fn resolved_payload_with_facts(
    ...
    ResolvedPayload::new(
        ExpandedFootprint::from_transaction(tx, expanded_dependencies, 64)
            .expect("fixture footprint is bounded"),
        CandidateMetrics {
            fee,
...
fn verify_remote_transaction_with_payload(
    ...
    let edges = payload.footprint.edge_count();
    let admission = ValidatedAdmission::remote(transaction, PeerIndex::from(peer), edges)
        .expect("fixture admission is valid");
    let hash = admission.identity.raw.clone();
    apply_without_work(
        authority
    ...
    apply_without_work(
        authority
            .plan_settlement(resolve.into_verify(payload).verified())
    ...
    for (root, root_entry) in &accepted {
        ...
        while let Some(descendant) = frontier.pop_front() {
            ...
            aggregate.fee = aggregate
                .fee
                .safe_add(entry.verified.payload.metrics.fee)
        ...
        let cost = root_entry.verified.payload.metrics.cost;
        let self_rate = FeeRate::calculate(
            root_entry.verified.payload.metrics.fee,
...
fn uak_all_four_preaccepted_phases_are_closed_variants() {
    ...
    let phases = [
        ...
        PreAcceptedPhase::Computed(ComputedOutcome::Verified(VerifiedFacts {
            ...
            payload: Arc::new(resolved_payload(&tx(0))),
        })),
    ];
...
fn uak_expanded_footprint_is_canonical_bounded_and_role_aware() {
    ...
    assert_eq!(footprint.dependencies(), &[dependency, declared_dependency]);
    ...
    let payload = ResolvedPayload::new(
        footprint.clone(),
        CandidateMetrics {
...
fn uak_popular_dependency_appends_sparse_reader_edges() {
    ...
    bounded.accepted =
        AcceptedResources::new(READER_COUNT + 1, 1024 * 1024, 1024 * 1024, 1024 * 1024);
    let mut authority = TxPoolAuthority::new(bounded);
...
fn uak_resource_batch_is_a_commutative_set_transition() {
    ...
    let mut ledger = ResourceLedger::new(ResourceLimits {
        preaccepted: unbounded,
        remote: unbounded,
        per_peer: unbounded,
        accepted: AcceptedResources::new(usize::MAX, usize::MAX, usize::MAX, u64::MAX),
    });
...
fn uak_independent_classifier_routes_pairwise_edges_without_mutation() {
    ...
    let right_tx = TransactionBuilder::default()
        ...
        .input(CellInput::new(shared_input.clone(), 0))
        .build();
    let mut conflicts = TxPoolAuthority::new(limits());
    ...
    let reader_tx = TransactionBuilder::default()
        ...
        .build();
    let mut conditional = TxPoolAuthority::new(limits());
...
fn uak_independent_capacity_is_aggregate_and_never_partially_applied() {
    let mut bounded = limits();
    bounded.accepted = AcceptedResources::new(1, 64 * 1024, 64 * 1024, 64);
    let mut authority = TxPoolAuthority::new(bounded);
    let first_tx = tx(215);
...
fn uak_independent_classifier_routes_every_accepted_relation_without_mutation() {
    ...
    let victim_tx = TransactionBuilder::default()
        ...
        .build();
    let mut conflict = TxPoolAuthority::new(limits());
    ...
    let parent_tx = TransactionBuilder::default()
        ...
        .output_data(Bytes::new().pack())
        .build();
    let mut causal = TxPoolAuthority::new(limits());
    ...
    let late_child_tx = TransactionBuilder::default()
        ...
        .build();
    let mut late = TxPoolAuthority::new(limits());
...
fn uak_coupled_late_parent_capacity_evicts_from_the_projected_graph() {
    let mut bounded = limits();
    bounded.accepted = AcceptedResources::new(2, 64 * 1024, 64 * 1024, 64);
    let mut authority = TxPoolAuthority::new(bounded);
    let parent_tx = TransactionBuilder::default()
        .version(231u32)
        .output(CellOutput::default())
...
fn uak_late_parent_component_bound_fails_before_authority_mutation() {
    let mut bounded = limits();
    bounded.accepted = AcceptedResources::new(128, 1024 * 1024, 1024 * 1024, 1024);
    let mut authority = TxPoolAuthority::new(bounded);
    let parent_tx = TransactionBuilder::default()
        .version(234u32)
        .output(CellOutput::default())
...
fn uak_late_parent_cannot_bypass_the_descendant_ancestor_bound() {
    let mut bounded = limits();
    bounded.accepted = AcceptedResources::new(130, 8 * 1024 * 1024, 8 * 1024 * 1024, 1024);
    let mut authority = TxPoolAuthority::new(bounded);
    let late_parent_tx = TransactionBuilder::default()
        .version(400u32)
        .output(CellOutput::default())
...
fn uak_rbf_component_bound_stops_before_any_authority_mutation() {
    let mut bounded = limits();
    bounded.accepted = AcceptedResources::new(110, 8 * 1024 * 1024, 8 * 1024 * 1024, 64);
...
fn uak_active_work_backpressure_is_precomputed_and_mutation_free() {
    let limits = ResourceLimits {
        preaccepted: ResourceVector::new(4, 64 * 1024, 64, 4),
        remote: ResourceVector::new(4, 64 * 1024, 64, 4),
        per_peer: ResourceVector::new(4, 64 * 1024, 64, 1),
        accepted: AcceptedResources::new(4, 64 * 1024, 64 * 1024, 64),
    };
    let mut authority = TxPoolAuthority::new(limits);
...
fn uak_checkout_is_move_only_and_exactly_charged() {
    ...
    let verify = resolve.into_verify(payload);
...
fn uak_continuation_yield_returns_one_queued_owner() {
    ...
    apply_without_work(
        authority
            .plan_settlement(resolve.yield_verify(payload))
...
fn uak_stale_lease_is_mutation_free_across_chain_epoch_and_token_mismatch() {
    ...
    let payload = resolved_payload(resolve.transaction());
    let settlement = resolve.into_verify(payload).internal_failure();
    authority.force_chain_epoch(ChainEpoch(1));
    let before = authority.normalized_snapshot();
    assert_eq!(
    ...
    let mut forged = second.into_verify(payload).verified();
    ...
    apply_without_work(
        authority
            .plan_settlement(
                resolve
                    .into_verify(payload)
                    .rejected(RejectionKind::Verification),
            )
