...
fn uak_direct_validation_rejection_ignores_an_unrelated_accepted_commit() {
    ...
    drop(
        authority
            ...
            .expect("an unrelated Accepted owner preserves the exact final read receipt"),
    );
}
...
fn uak_location_refresh_rechecks_the_configured_minimum_fee() {
    ...
    let FinalAdmissionDispositionPlan::ValidationRejected(plan) = authority
        .plan_final_admission(outcome)
        .expect("the refreshed low-fee outcome has one terminal compiler")
    else {
        panic!("a refreshed low-fee transaction must not reach membership");
    };
    let (_, committed) = plan.apply();
    assert_eq!(committed.retired_len(), 1);
