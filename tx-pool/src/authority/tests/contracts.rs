...
fn merging_complete_captures_accepts_identical_versions_and_preserves_conflicts() {
    let store = store();
    for (capture, nonce) in [Store::capture_all, Store::capture_accepted]
        .into_iter()
        .zip([7_810, 7_811])
    {
        let Captured { view, reads, .. } = capture(&store);
        let mut original = ReadSet::default();
        original.merge(&reads).unwrap();
        original.merge(&reads).unwrap();
        store.read_selected(view, &original, || ()).unwrap();

        accept(&store, tx(nonce), 1, 1, Status::Pending);
        let fresh = capture(&store).reads;
        assert!(matches!(original.merge(&fresh), Err(Error::Stale)));
        assert!(matches!(
...
fn accepted_capture_ignores_prepool_changes_but_detects_accepted_edits() {
    ...
    let Captured { view, reads, .. } = store.capture_accepted();
...
fn generation_clear_invalidates_old_reads_even_when_the_snapshot_is_identical() {
    ...
    old.edit(None, Some(entry(&store, tx(8), Source::Local)), None)
        .unwrap();
    store
        .apply(chain::clear(&store, Some(Arc::clone(&snapshot)), ClearScope::All).unwrap())
        .unwrap();
...
fn rejected_multi_owner_edit_has_no_visible_prefix_or_accounting_change() {
    ...
    assert_eq!(store.capture_all().owners.len(), 2);
...
fn worker_notifications_require_queued_work_or_returned_capacity() {
    ...
    lifecycle.reset(store.snapshot().1);
