...
impl ObservedDependencies {
    ...
    pub(super) fn observed(&self) -> &KnownDependencies {
        &self.observed
    }
...
impl ValidatedAdmission {
    ...
    /// Recompile an existing preaccepted owner into a fresh generation
    /// without erasing its ingress provenance or its original accounting
    /// basis. A Recovery lease is generation-scoped and therefore rebinds;
    /// Remote and Proposal provenance remain the same external fact.
    pub(super) fn generation_requeue(
        entry: PreAcceptedEntry,
        generation: PoolGeneration,
    ) -> (Self, ResourceVector) {
        let PreAcceptedEntry {
            record,
            source,
            basis,
            phase: _,
        ...
        let source = match source {
            ...
            source @ (PreAcceptedSource::Remote(_) | PreAcceptedSource::Proposal { .. }) => source,
        };
        let original_charge = basis.charge();
        (
            Self {
                tx: record.tx,
                identity: record.identity,
                source,
                dependencies: basis.declared_dependencies,
                payload_bytes: basis.payload_bytes,
                encoded_edges: basis.encoded_edges,
            },
            original_charge,
        )
    }
