...
pub(super) struct RelayParentRebuildCut {
    generation: PoolGeneration,
    shards: [u64; super::shard::AUTHORITY_SHARD_COUNT],
}
...
impl<'authority> AuthorityReadView<'authority> {
    ...
    pub(super) fn capture_relay_parent_rebuild(
        ...
        let cut = RelayParentRebuildCut {
            generation: self.generation,
            shards: owners.relay_parent_sources(),
        };
        ...
        let has_more = owners.remote_page_into(after.as_ref(), limit.get(), &mut scratch.remote)?;
