...
use super::{
    ...
    source::{PoolTemplateVersions, TemplateSelectionSource},
    state::{
...
impl TemplateSourceCut {
    ...
    fn covers(self, target: Self) -> bool {
        self.pool.0.proposals.covers(target.pool.0.proposals)
            && self.pool.0.transactions.covers(target.pool.0.transactions)
...
impl TemplatePoolSourceCut {
    ...
    fn join(self, incoming: Self) -> Self {
        Self(PoolTemplateVersions {
            proposals: self.0.proposals.join(incoming.0.proposals),
            transactions: self.0.transactions.join(incoming.0.transactions),
...
pub(super) struct ProposalSourceCut {
    selection: TemplateSelectionSource,
    chain: ApplySequence,
}
...
pub(super) struct UncleSourceCut {
    ...
    chain: ApplySequence,
    proposals: TemplateSelectionSource,
}
