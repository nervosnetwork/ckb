...
impl ComputeWorkerSlot {
    ...
    /// Stable role order used by one compute wave. The ordered resolver is
    /// first, then Small-only verifiers, then Any verifiers; worker id is only
    /// a deterministic tie-break among equivalent roles.
