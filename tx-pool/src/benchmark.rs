...
///
/// On drop it drains the retained relay endpoint while cancelling and awaiting
/// the generation-owned dispatcher. The receiver and persistence directory
/// outlive that join, so teardown neither backpressures the bounded effect log
/// nor exposes a prematurely disconnected endpoint or removed path.
...
impl Drop for ServiceHandle {
    fn drop(&mut self) {
        ...
        if let Some(receiver) = self.relay_receiver.take() {
        ...
        if let Some(dispatcher) = self.dispatcher_handle.take() {
            ...
            handles.push(self.runtime.spawn(async move {
                ...
                stop.cancel();
                result.expect("benchmark dispatcher task joins during teardown");
            }));
        } else {
            drain_stop.cancel();
        }
