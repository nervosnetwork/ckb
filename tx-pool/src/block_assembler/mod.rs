...
impl BlockAssembler {
    ...
    pub(crate) fn build_base_template(
        ...
        if fixed_size > Self::max_block_bytes(&snapshot)? {
    ...
    /// Fit the sole driver's optional content. Proposal liveness has byte priority; only
    ...
    /// template so the proposal remains present if a miner drops optional
    /// uncles. Descendants of an omitted uncle are also omitted unless
