...
use ckb_types::{
    core::{BlockNumber, EpochExt, UncleBlockView},
    packed::Byte32,
};
...
impl BoundedCandidateUncle {
    pub(crate) fn payload_limit(max_block_bytes: u64) -> Option<usize> {
        usize::try_from(max_block_bytes)
            .ok()?
            .checked_add(Byte32::TOTAL_SIZE)
    }
...
struct CandidateInsertion {
    ...
    next_version: CandidateUncleSourceReceipt,
}
...
pub struct CandidateUncles {
    ...
    source_version: CandidateUncleSourceReceipt,
}
...
impl CandidateUncles {
    ...
    pub fn insert(&mut self, uncle: UncleBlockView) -> bool {
        ...
        // production channel and detached-chain recovery both pass bounded
        // candidates to `try_insert_bounded`.
