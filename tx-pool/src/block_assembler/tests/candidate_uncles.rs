...
fn candidate_uncle_source_version_exhaustion_is_typed_and_mutation_free() {
    let mut candidates = CandidateUncles::new();
    candidates.source_version = CandidateUncleSourceReceipt(u64::MAX);
    let block = uncle(0, 0);
    ...
    assert_eq!(
        ...
        CandidateUncleSourceReceipt(u64::MAX)
    );

    candidates.source_version = CandidateUncleSourceReceipt(0);
    assert!(
        candidates
