...
async fn ordinary_compute_and_template_waits_preserve_reserved_read_routes() {
    ...
    let client = controller.clone();
    let snapshot = tokio::task::spawn_blocking(move || client.input_snapshot());
    let Message::GetInputSnapshot(request) =
        tokio::time::timeout(Duration::from_secs(5), queries.recv())
            .await
            .unwrap()
            .unwrap()
    else {
        panic!("indexer snapshots keep reserved read capacity");
    };
    request
        .responder
