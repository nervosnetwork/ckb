...
pub(crate) async fn process(
    ...
    match message {
        ...
        Message::RemoveLocalTx(request) => {
            ...
            respond_local_removal(responder, service.remove_local(&hash).await)
        }
...
fn respond_local_removal<S>(
    responder: S,
    result: Result<
        Result<bool, crate::service::LocalRemovalCompetingProgress>,
        AuthorityServiceError,
    >,
) -> Result<(), AuthorityGenerationInvalidity>
where
    S: OneshotSender<Result<bool, AnyError>>,
{
    match result {
        Ok(Ok(removed)) => {
            respond(responder, Ok(removed), "remove_local_tx");
            Ok(())
        }
        Ok(Err(competing)) => {
            respond(responder, Err(AnyError::from(competing)), "remove_local_tx");
            Ok(())
        }
        Err(error) => {
            respond(
                responder,
                Err(authority_error_as_any(&error)),
                "remove_local_tx",
            );
            settle_service_error(error)
        }
