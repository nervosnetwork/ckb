...
#[test]
fn persistence_prepares_parent_first_replay_for_both_file_versions() {
    ...
    for version in [LEGACY_VERSION, VERSION] {
        ...
        if version == LEGACY_VERSION {
            let vector = TransactionVec::new_builder()
                .extend([child.data(), parent.data()])
                .build();
            std::fs::write(versioned_path(&base, version), vector.as_slice()).unwrap();
        } else {
            write_snapshot(
                &base,
                PersistenceSnapshot {
                    accepted: vec![child.clone()],
                    recovery: vec![parent.clone()],
                },
            )
            .unwrap();
        }
        let prepared = load_persistence_snapshot(&config(&base))
            ...
            .unwrap();
        assert_eq!(prepared, vec![parent.clone(), child.clone()]);
    }
}
