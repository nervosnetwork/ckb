use super::ResolvedOverlayCellChecker;
use ckb_types::{
...
#[test]
fn resolution_evidence_skips_same_tip_chain_revalidation() {
    let tip = Byte32::new([1; 32]);
    let overlay = ProbeChecker::new(None);
    let chain = ProbeChecker::new(Some(false));
    let checker = ResolvedOverlayCellChecker::new(&overlay, &chain, &tip, &tip);
    let cell = resolved_chain_cell(OutPoint::new(Byte32::new([2; 32]), 0));

    assert_eq!(checker.is_live_cell(&cell), Some(true));
    assert_eq!(overlay.calls.get(), 1);
    assert_eq!(chain.calls.get(), 0);
}

#[test]
fn resolution_evidence_yields_to_pool_spends_and_tip_changes() {
    ...
    let same_tip = ResolvedOverlayCellChecker::new(
        &spent_overlay,
        &unused_chain,
        &resolved_tip,
        &resolved_tip,
    );
    assert_eq!(same_tip.is_live_cell(&cell), Some(false));
    assert_eq!(unused_chain.calls.get(), 0);

    let empty_overlay = ProbeChecker::new(None);
    let changed_chain = ProbeChecker::new(Some(false));
    let changed_tip = ResolvedOverlayCellChecker::new(
        &empty_overlay,
        &changed_chain,
        &resolved_tip,
        &current_tip,
    );
    assert_eq!(changed_tip.is_live_cell(&cell), Some(false));
    assert_eq!(changed_chain.calls.get(), 1);
}

#[test]
fn resolution_evidence_requires_chain_provenance() {
    let tip = Byte32::new([1; 32]);
    let overlay = ProbeChecker::new(None);
    let chain = ProbeChecker::new(Some(false));
    let checker = ResolvedOverlayCellChecker::new(&overlay, &chain, &tip, &tip);
    ...
    assert_eq!(checker.is_live_cell(&cell), Some(false));
