...
pub trait PoolRpc {
    ...
    /// * `outputs_validator` - Validates the transaction outputs before entering the tx-pool.
...
impl PoolRpc for PoolRpcImpl {
    fn send_transaction(
        ...
        let broadcast = |tx_hash: packed::Byte32| {
            // workaround: we are using `PeerIndex(usize::max)` to indicate that tx hash source is
            // itself.
