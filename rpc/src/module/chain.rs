...
use ckb_jsonrpc_types::{
    ...
    Transaction, TransactionProof, TransactionWithStatusResponse, Uint32, Uint64, WitnessesProof,
};
...
pub trait ChainRpc {
    ...
    /// Returns a Merkle proof of transactions' witnesses are included in a block.
    ///
    /// ## Params
    ///
    /// * `tx_hashes` - Transaction hashes, all transactions must be in the same block
    ...
    /// }
    /// ```
    #[rpc(name = "get_witnesses_proof")]
    fn get_witnesses_proof(
        &self,
        tx_hashes: Vec<H256>,
        block_hash: Option<H256>,
    ) -> Result<WitnessesProof>;

    /// Verifies that a proof points to transactions in a block, returning the transaction hashes it commits to.
    ///
    /// ## Parameters
    ///
    /// * `witnesses_proof` - proof generated by [`get_witnesses_proof`](#tymethod.get_witnesses_proof).
    ///
    /// ## Examples
    ///
    /// Request
    ...
    /// }
    /// ```
    #[rpc(name = "verify_witnesses_proof")]
    fn verify_witnesses_proof(&self, tx_proof: WitnessesProof) -> Result<Vec<H256>>;
    ...
    fn get_proof_common_0(
        ...
        snapshot
            ...
            .map(|block| {
                ...
                let leaves = match get_type {
                    GetWitnessesProof => block.tx_witness_hashes().to_vec(),
                    GetTransactionProof => block.transactions().iter().map(|tx| tx.hash()).collect::<Vec<_>>(),
                };
                (block.hash().unpack(), witnesses_root.unpack(),CBMT::build_merkle_proof(&leaves, &tx_indices.into_iter().collect::<Vec<_>>()).expect("build proof with verified inputs should be OK"))
            })
    }
}
