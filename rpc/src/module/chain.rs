...
use crate::module::chain::GetProofType::{GetTransactionProof, GetWitnessesProof};
...
use ckb_jsonrpc_types::{
    ...
    Transaction, TransactionProof, TransactionWithStatusResponse, Uint32, Uint64, WitnessProof,
};
...
pub trait ChainRpc {
    ...
    ///     "result": {
    ///         "block_hash": "0x7978ec7ce5b507cfb52e149e36b1a23f6062ed150503c85bbf825da3599095ed",
    ///         "raw_transactions_root": "0xa4037a893eb48e18ed4ef61034ce26eba9c585f15c9cee102ae58505565eccc3",
    ///         "witnesses_proof": {
    ///             "indices": [
    ///                 "0x0"
    ///             ],
    ...
    /// }
    /// ```
    #[rpc(name = "get_witness_proof")]
    fn get_witness_proof(
        &self,
        tx_hashes: Vec<H256>,
        block_hash: Option<H256>,
    ) -> Result<WitnessProof>;
    ...
    /// ## Parameters
    ///
    /// * `witness_proof` - proof generated by [`get_witness_proof`](#tymethod.get_witness_proof).
    ///
    /// ## Examples
    ///
    /// Request
    ...
    /// }
    /// ```
    #[rpc(name = "verify_witness_proof")]
    fn verify_witness_proof(&self, tx_proof: WitnessProof) -> Result<Vec<H256>>;
...
impl ChainRpc for ChainRpcImpl {
    ...
    fn get_transaction_proof(
        ...
        let (block, witnesses_root, proof) =
    ...
    fn get_witness_proof(
        ...
        let (block, raw_transactions_root, proof) =
            self.get_proof_common_0(tx_hashes, block_hash, GetWitnessesProof)?;
        Ok(WitnessProof {
            block_hash: block,
            raw_transactions_root,
            witnesses_proof: JsonMerkleProof {
                indices: proof
                    .indices()
                    .iter()
                    .map(|index| (*index).into())
    ...
    fn verify_witness_proof(&self, tx_proof: WitnessProof) -> Result<Vec<H256>> {
        ...
        snapshot
            ...
            .and_then(|block| {
                let raw_transactions_root = tx_proof.raw_transactions_root.pack();
                let merkle_proof = MerkleProof::new(
                    tx_proof
                        .witnesses_proof
                        .indices
                ...
                CBMT::retrieve_leaves(block.tx_witness_hashes(), &merkle_proof)
                    .and_then(|tx_hashes| {
                        merkle_proof
                            .root(&tx_hashes)
                            .and_then(|raw_witnesses_root| {
                                if block.transactions_root()
                                    == merkle_root(&[raw_transactions_root, raw_witnesses_root])
                                {
    ...
    fn get_proof_common_0(
        ...
        snapshot
            ...
            .map(|block| {
                match get_type {
                    GetTransactionProof => {
                        (block.hash().unpack(),block.calc_witnesses_root().unpack(),CBMT::build_merkle_proof(&block.transactions().iter().map(|tx| tx.hash()).collect::<Vec<_>>(), &tx_indices.into_iter().collect::<Vec<_>>()).expect("build proof with verified inputs should be OK"))
                    },
                    GetWitnessesProof => {
                        (block.hash().unpack(),block.calc_raw_transactions_root().unpack(),CBMT::build_merkle_proof(block.tx_witness_hashes(), &tx_indices.into_iter().collect::<Vec<_>>()).expect("build proof with verified inputs should be OK"))
                    }
                }
            })
    }
